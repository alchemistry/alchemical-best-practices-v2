...
Eq.~(\ref{eq:intra-zu-def}) describes the intramolecular configurational partition functions $z_{R(v)}$ and $z_{L(v)}$ for the receptor $R$ and ligand $L$. The intramolecular configurational partition function $z_{RL(v)}$ of the complex is similar, but the integration is limited to a chosen range of configurations in which receptor and ligand are considered ``bound''.
...
Equivalently, one could calculate $K_b = \frac{k_{on}}{k_{off}}$, as %MRS: add to this
...
If you like playing with Taylor series, you can also show that Eq.~\label{eq:TI_defn} can be derived by taking the Zwanzig equation,
...
\begin{equation}
\Delta G_{\mathrm{slv, exc.}} = - k_B T \ln \frac{z_{A(v)}}{z_{A(\mathrm{g})}} = k_B T \ln \frac{k_H}{k_B T} \, .
\label{eq:DG-solvation-excess-app}
\end{equation}
Eq.~(\ref{eq:DG-solvation-excess-app}), which involves only intramolecular partition functions (as described above),  states that the excess free energy of solvation corresponds to the process of moving the solute from a fixed position and orientation in the gas to a fixed position and orientation in the solvent. This is sometimes referred to as the solvation free energy in the Ben-Naim standard state~\cite{bennaim1984solvation}.
