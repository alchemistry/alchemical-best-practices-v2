...
The definition of macrostates that are defined by specific choices of the microstates is required in many areas of computational chemistry and biophysics, for example, when specifying the range of backbone angles to measure the relative populations of $\alpha$-helical and $\beta$-strand conformations of peptides, or even whether a protein is folded or not.  What these microstates have in common is that they are different functionally. Only certain configurations have biological or experimental effects, and thus when we analyze any sort of simulation, we should try to figure out what simulation variables correspond to the experimentally configurations that have the measured effect. 

This interpretation of experimental reporters of binding is a complex issue in itself.  For example, an experimental method that identifies any energetic interaction between a protein and a ligand (such as isothermal titration calorimetry) will indicate a different bound state than one that only detects a ligand that is positioned in a specific location on the protein (such as changes in polarization upon displacement of a probe ligand). The relationship between the predictions of this theory and experimental reporters (thermochemical, spectroscopic, inhibition kinetics, and others) of complex formation has been discussed~\cite{gilson1997statisticalthermodynamic,mihailescu2004theory,gallicchio2011recent,simonson2016physical,gallicchio2021comppeptsci}. All the different definitions of the bound state yield nearly equivalent values for the free energy when binding is strong and specific, and the definition of the bound complex encompasses the major bound poses while excluding obvious unbound configurations. Weaker, less specific interactions, as discussed in Sec.~\ref{sec:weak-binders} require somewhat more work to define.  As mentioned above, in some cases it is valuable to consider specific definitions of the complex, for example, by restricting the ligand's orientation to pinpoint the relative contributions of different binding poses.
...
While it is generally not feasible to compute the two partition functions $Z(\lambda)$, several estimators have been devised to robustly estimate the ratio of partition functions in Eq.~\ref{eq:delta-G-env} (see Sec.~\ref{subsec:estimators}) from a set of configurations usually collected with MD simulations from the thermodynamic states defined at $\lambda_A$ and $\lambda_B$ and intermediates in between these two.  For simplicity, we will frequently use the more specific $\beta U(\vec{q})$ instead of $u(q\vec{q})$, but for more complicated situations where one is dealing with simulations at multiple temperatures or pressures, it can prove useful. 

\paragraph{Zwanzig relationship or exponential averaging connects ratios of partition functions to expectation values of energy differences}
...
occasionally if the energy goes to infinity.
  ...
  and $Z_B = e^{-f_B}$.   So we can say that 
...
free energy of ligand binding is more complicated, because there are.
We cover in Sec.\ref{subsec:formulations-abfe-rbfe} how to set up the proper partition
...
expectation values.  Once we have defined $U(\vec{q},\lambda)$, as
...
\begin{equation}
    \Delta G = k_BT \Delta f = k_BT \left(\sum_{k=0}^{K-1} \Delta f_{k,k+1}\right)
\end{equation}
where
\begin{equation}
...
Which we can compute from samples collected at all the alchemical states $\{\lambda_k \}$, and where $\Delta f$ is the \emph{unitless free energy difference}.
...
A third approach borrows ideas from simulated tempering~\cite{marinari1992simulated}. In this scheme, a single replica rapidly explores all of $\lambda$ space by switching between different intermediate $\lambda$ values, as seen in Fig.~\ref{fig:fig_sampling_scheme}\textbf{C}.  This is referred to as expanded ensemble, extended ensemble or even just simulated tempering (although the temperature is not changed when applied to $\lambda$ changes in alchemical free energy calculations).~\cite{lyubartsev1992new, li2007simulated, tan2017optimally} and is supported by OpenMM Tools~\cite{andrearizzi2019choderalab}, GROMACS and AMBER TI~\cite{Lee_JPhysChemB_2025_v129_p13063}.    One can show that the expanded ensemble calculations will move back and forth between replicas faster than replica exchange.~\cite{zhang2008comparison,hsu2024replica}  
...
We define this intramolecular partition function more precisely here, as:
\begin{equation}
...
\textbf{MRS}: Helped create figure~\ref{fig:fig_what_is_lambda}, wrote Sec.~\ref{sec:important_path} describing choices for alchemical pathways and parts of~\ref{sec:data_analysis} on the analysis for free energy calculations. Reviewed, organized and edited text throughout.\\
