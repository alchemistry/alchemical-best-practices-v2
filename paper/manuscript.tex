...
\end{equation}
where $\Omega$ denotes the state space and $f : \Omega \rightarrow \mathbb{R}$ is an observable. Even though not all estimators can be written in the exact form Eq.~\ref{eqn:empirical-average}, it will be sufficient to consider this case to get the main idea. The key object of study in this section is an estimator of 
\begin{equation}
...
Consider a molecular dynamics trajectory started at some initial configuration $z_0 = (\mathbf{q}_0, v_0)$, where $\mathbf{q}_0 \in \mathbb{R}^{3 N}$ denotes the initial positions of the system and $v_0 \in \mathbb{R}^{3 N}$ its initial velocities, and denote the density at time $t$ by $\rho_t(z|z_0)$. This transition density is determined by the molecular dynamics integrator. The distribution of the velocities is usually ignored, since in the presence of a thermostat they are kept distributed roughly according to a Maxwell-Boltzmann distribution, so we only consider the $\mathbf{q}$ component. In principle, one could consider the initial condition $\mathbf{q}_0$ to be drawn from a more complicated initial distribution $\rho_0(\mathbf{q}) \mathrm{d}\mathbf{q}$, like a mixture of Dirac densities $\rho_0(\mathbf{q}) \mathrm{d}\mathbf{q} = \sum_{i = 1}^n \delta_{\{ \mathbf{q}_i \}}(\mathrm{d}\mathbf{q}) / n$, but any such "better" initial density would require some knowledge about the system at hand, such as the distribution of binding modes. 
...
% \footnote{(Luckily, it is not necessary for simulations to visit every configuration $\mathbf{q} \in \Omega$ in order to exhibit a regime of convergence for the free energy estimates. For instance, two configurations $\mathbf{q}$ and $\mathbf{q}'$ may be considered equivalent for free energy estimation if they differ only by the permutation of water molecules. Nature too faces the same difficulty that not all states are accessible on timescales we can observe -- it will take an astronomically long time before all the gas particles in a room happen to be on one half-side of it -- but this does not prevent us from measuring the temperature of (most) rooms.)}
...
\textbf{Definition}:
We define an \textbf{ensemble} to be a set of $R \geqslant 1$ trajectories, all started at the same initial configuration $\mathbf{q}_0$, with initial velocity $v_0$ independently drawn from the Maxwell-Boltzmann distribution at the appropriate temperature and run for a chemical time $T$. We emphasize that, with this definition, all ensemble-based estimates are inherently tied to the choice of $\mathbf{q}_0$; pathological choices  -- for example, a protein–ligand configuration that may quickly unbind -- can render the resulting estimates meaningless.

\subsubsection{Ensemble error estimates}

This section describes the simplest and, in many cases, the most robust procedure for estimating uncertainties in free energy calculations. For most practical applications, we recommend this approach when reporting statistical error bars. The procedure consists of simulating an ensemble $\{X^{(1)},\ldots,X^{(R)}\}$ of $R$ molecular dynamics trajectories initialized at $\mathbf{q}_0$, and using the resulting free energy estimates $\{\Delta G^{(1)},\ldots,\Delta G^{(R)}\}$ to quantify uncertainty. The trajectories $\Delta
G^{(i)} (\cdot)$ are identically distributed and conditionally independent given $\mathbf{q}_0$. For any time $T > 0$, the variance of any $\Delta G^{(i)} (T)$ can be estimated with the sample variance $S^2_{R} (T)$,
...
\end{equation}
In the notation of Eq.~\ref{eqn:empirical-average}, $Z_i = \Delta G^{(i)}(T)$; conditioned on $\mathbf{q}_0$, the $Z_i$ are independent and identically distributed and are trivially a Markov process, with stationary distribution given by the distribution of $\Delta G^{(1)}(T)$. By direct calculation, we have (conditionally on $\mathbf{q}_0$)
\begin{equation}
    \mathbb{E}_{\mathbf{q}_0}[S_R^2(T)] = \mathrm{Var}_{\mathbf{q}_0}(\Delta G^{(1)}(T))
\end{equation}
The above equation says that $S_R^2(T)$ is an unbiased estimator of the variance of $\Delta G^{(1)}(T)$, conditioned on $\mathbf{q}_0$. By the Law of Large Numbers, as $R \rightarrow \infty$, we have $S^2_R (T) \rightarrow \mathrm{Var} _{\mathbf{q}_0}(\Delta
G^{(1)} (T)) = \sigma^2 (T ; \mathbf{q}_0)$. Furthermore, by conditional independence of the $\Delta G^{(i)}(T)$, for sufficiently large $R$ and any $T>0$ for which the variance is finite, we have
\begin{equation}\label{eqn:variance-of-empirical-mean}
\mathrm{Var}_{\mathbf{q}_0}(\overline{\Delta G(T)}) = \frac{\mathrm{Var}_{\mathbf{q}_0}(\Delta G^{(1)}(T))}{R} \approx \frac{S_R^2(T)}{R}
\end{equation}
...
\begin{equation}
  \mathrm{SE}_R (T ; \mathbf{q}_0) = \sqrt{\frac{S_R^2(T)}{R}},
\end{equation}
which is the gold standard for reporting the error of a free energy estimate $\overline{\Delta G (T)} $ when a single initial condition $\mathbf{q}_0$ is used. Note that $\mathrm{SE}_R
(T ; \mathbf{q}_0) \rightarrow 0$ as $R \rightarrow \infty$ so long as $\mathrm{Var}_{\mathbf{q}_0}(\Delta G^{(1)}(T))$ is finite, which is true if the $\Delta G^{(1)}(T)$ are bounded. Thus the variance can be made arbitrarily small simply by running enough replicates, but we note critically this does \textit{not} mean that we will have estimated the free energy to arbitrary precision: $\Delta G^{(1)}(T)$ in general will not be normally distributed, its value may vary greatly with both $T$ and $\mathbf{q}_0$, and the variance may not be a good statistic to capture the error, as the distribution may be multi-modal, heavy-tailed and be sensitive to rare events. For example, one replicate out of many might explore a different mode of an important torsional angle, leading to a drastically different free energy estimate. 
...
In the preceding case, we used minimal assumptions on the distribution of $\Delta G^{(i)}(T)$ to obtain estimates on $\sigma^2(T;\mathbf{q}_0)$ by simulating conditionally independent replicates and using the sample variance Eq.~\ref{eq:sample-variance-definition}. To obtain per-replicate (i.e., per-$i$) error estimates, we impose much stricter assumptions on the distribution of $\Delta G^{(i)}(T)$,  namely that they satisfy a Central Limit Theorem (CLT) as $T\rightarrow \infty$, for each $i=1,\dots,R$:
$$ \sqrt{T} ( \Delta G^{(i)}(T) - \Delta G^\star) \Rightarrow N(0, \sigma^2_{\mathrm{asym}}(\mathbf{q}_0))$$
...
$$\sigma^2(T;\mathbf{q}_0)\approx\frac{\sigma^2_{\mathrm{asym}}(\mathbf{q}_0)}{T}.$$
In practice, it may not be the case that $\sigma^2(T;\mathbf{q}_0)\propto T^{-1}$, nor even that $\sigma^2(T;\mathbf{q}_0)$ is monotonically decreasing with $T$. In such a case, we are not in a regime where the CLT is satisfied and the remainder of this subsection does not apply.

To understand the difficulty of estimating $\mathrm{Var}_{\mathbf{q}_0}(\Delta G^{(i)}(T))$ using only samples from replicate $i$, it is helpful to recall the empirical mean Eq.~\ref{eqn:empirical-average} as the canonical estimator. Suppose that $\{Z_i\}_{i\ge 0}$ is an ergodic Markov chain with stationary distribution $\pi$, that $Z_0 \sim \pi$, and that $t_{N-1}=T$. Let $f$ be a fixed observable. 
\begin{align}
\sigma^2(T;\mathbf{q}_0) &= \mathrm{Var}_{\pi}\!\left(\frac{1}{N}\sum_{i=0}^{N-1} f(Z_i) \right) \\
...
Partition the $N$ sequential samples of this single replicate into $B$ consecutive batches of equal size $M$, so that $N = BM$. Batch $k$ consists of the samples $\{X(t_{kM}),\dots,X(t_{(k+1)M-1})\}, k=0,\dots,B-1$, with $X(t_0) = \mathbf{q}_0$. Let
\[
...
$$ \hat\sigma_B^2(T; \mathbf{q}_0) \approx S_R^2(T ; \mathbf{q}_0) .$$
...
\begin{eqnarray}
g &\equiv& 1 + 2 \tau_\mathrm{eq} \approx  \frac{\sigma^2(T;\mathbf{q}_0)}{\sigma_0^2} N \label{eq:statistical-inefficiency-definition} 
\end{eqnarray}
is referred to as the \textit{statistical inefficiency}. Loosely speaking, $g$ measures the number of indices $i$ between roughly independent samples. The role of estimators such as non-overlapping batch means is precisely to estimate $\sigma^2(T;\mathbf{q}_0)$ directly from correlated data, which is roughly equivalent to estimating $g$. 
