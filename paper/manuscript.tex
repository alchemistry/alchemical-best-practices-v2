...
\end{equation}
where $\beta = (k_B T)^{-1}$, $\lambda_i$ is the thermal De Broglie wavelength of the $i$-th atom of the solute (not the alchemical parameter as used elsewhere in this document), and $z_u$ is the intramolecular configurational partition function of the solute. See a more complete definition of $z_u$ in Sec.~\ref{sec:derivations}.
...
As described before in~\ref{sec:theory}, the chemical potential of a solute $u$ in an ideally diluted solution in a solvent $v$ can be written as~\cite{guggenheim1952mixtures504,gilson1997statisticalthermodynamic,gilson2007calculation}
\begin{equation}
  \mu_u = -k_B T \ln \frac{q'_u}{C_u \Lambda_u^3}
...
The quantity $q'_u$ in Eq.~\ref{eq:chemical-potential-definition-app} is the molecular partition function of the solute at rest in the solvent
\begin{equation}
  q'_u = \frac{\Lambda_u^3}{\prod_i \lambda_i^3} 8 \pi^2 z_u
  \label{eq:intra-qpu-def-app}
\end{equation}
where $\beta = (k_B T)^{-1}$, $\lambda_i$ is the thermal De Broglie wavelength of the $i$-th atom of the solute (not the alchemical parameter as used elsewhere in this document), and $z_u$ is the intramolecular configurational partition function of the solute.  As described in Sec.~\ref{sec:theory}, masses do not enter the classical statistical mechanical potentials, so the thermal De Broglie wavelength terms coming from the partition functions of the momenta cancel in the resulting expressions below, and we will no longer consider them.  Up to this point, we are merely recapitulating what was presented in Sec.~\ref{sec:theory}; indeed, Eqs.~\ref{eq:chemical-potential-definition-app} and \ref{eq:intra-qpu-def-app} are the same as Eqs. ~\ref{eq:chemical-potential-definition} and \ref{eq:intra-qpu-def} and are included here again for completeness (similar repetitions will occur throughout this section).
...
\end{equation}
where $P_{sv} = C_{A(v)}/C_{A(s)}$ is the partition coefficient of $A$ for the two solvents~\cite{rustenburg2016measuring}. Eq.~\ref{eq:DG0-partitioning-def-app} 
immediately identifies the partition coefficient with the ratio of intramolecular configurational partition functions of solute $A$ in the two solvents:
\begin{equation}
  P_{sv} = \frac{z_{A(v)}}{z_{A(s)}} \, .
