...
While solutions to dummy atom challenges may in principle be known, most are not yet implemented in setup tools and free energy pipelines and have not yet been thoroughly characterized, so it seems likely that considerable science and engineering remains to be done in this space. 
 ...
 Absolute binding free energies often suffer from systematic errors arising from insufficient sampling of end states. Using an holo structure as the starting conformation artificially preorganizes the binding site, potentially overestimating binding affinity if the system does not relax to the apo state. This is an area of active research and tentative remedial approaches have been proposed for the error sources identified in ABFE studies. One common source of apo-state undersampling is inadequate water resolvation of the binding pocket. In such cases, a short GCMC like equilibration simulation with water swapping could be performed for each alchemical window to resolvate the pocket to known apo solvation levels~\cite{NithishABFE}. However, it must be noted that the GCMC runs must be applied per-window in the presence of the partially decoupled ligand to allow for enough configurational overlap across the lambda space\cite{ChenggongHuiGCMC}.  For membrane proteins with lipid-exposed binding pockets, slow lipid exchange represents an additional source of apo-state undersampling. In such cases Coarse-grained MD provides a means of identifying probable lipid binding sites by accelerating lipid dynamics. Once identified, the free energy of the lipid binding can be incorporated as a separate step in the alchemical cycle, as a post hoc correction. Third, and most broadly, protein conformational rearrangement upon ligand removal represents the most common and challenging manifestation of apo state error. One proposed remedy is to seed higher-lambda windows with apo-like structures ~\cite{SchrodingerVogele} ~\cite{CharlieCookABFE}, but this approach can produce erroneous affinities in the case of binders where the apo state was well sampled. An alternative is to characterize the relevant conformational states using geometric descriptors of the binding pocket \cite{SchrodingerVogele}, then apply enhanced sampling methods to estimate the reorganization free energy ($\Delta G _{holo \rightarrow apo}$) as an explicit thermodynamic contribution to the alchemical cycle ~\cite{NithishABFE}. A pragmatic alternative is to apply a reference-compound offset, this is reasonable only if the reorganization free energy is approximately uniform across the ligand series, an assumption that could break down with large conformational differences between binders.
