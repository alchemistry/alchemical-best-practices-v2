...
$T$ --- temperature \\
$\mathcal{T}$ --- length of a single trajectory\\
...
f^{(1)} (\mathcal{T})) = \sigma^2 (\mathcal{T} ; \mathbf{q}_0)$. Furthermore, by conditional independence of the $\Delta f^{(i)}(\mathcal{T})$, for sufficiently large $N_{\mathrm{rep}}$ and any $\mathcal{T}>0$ for which the variance is finite, we have
