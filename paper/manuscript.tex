...
In this section, we introduce the necessarery thermodynamic and statistical mechanics theory of dilute solutions to define the quantities we are trying to calculate using alchemical methods, specifically, solvation, solvent partitioning equilibria and noncovalent molecular binding. The emphasis here is placed on bridging theoretical foundations and intuition. We cover these derivation in more detail in Sec.~\ref{sec:derivations}, as there are important subtleties in the definitions that require very careful analysis.  This section is intended as an overview, and will involve a little bit of handwaving.
...
However, often what we really need to know is not the free energy of binding, but how much better one ligand is binding than another ligand.  This is really all we need to know in lead optimization, where we seek to find a new compund somewhat different from a reference compound, but that binds more tightly.  Relative free energy calculations avoid the need to simulate binding and unbinding events by making use of the fact that the free energy is a state function and exploiting the thermodynamic cycle illustrated in Fig.~\ref{fig:fig_binding_thermodynamic_cycle}.  Fig.~\ref{fig:fig_binding_thermodynamic_cycle} tell us that the difference in free energy of binding between toluene (A)and benzylalcohol (B) can bec omputed by running two independent calculations estimating the free energy cost of mutating A into B in the binding pocket($\Delta G_{bound}$) and in solvent($\Delta G_{unbound}$), saving us the need to simulate the physical binding process of the two compounds.  A significant amount of the remainder of the paper will specifically look at the different choices that need to be made when electing to calculate either absolute or relative free energies of binding.
...
The general consensus is that relative binding free energies are more accurate ways to calculate, most likely because of various types of cancellation of errors. Absolute free energies involve larger pertubations in the system, as one needs to remove and add more atoms from the system. Additionally, if there are errors in the force fields describing the protein and the ligands, any errors in interactions that are conserved between the two ligands will tend to cancel out in the results.
...
When we carry out a properly run molecular dynamcs or Monte Carlo
...
are generating samples consistent with the Boltzmann distrbution
$p(\vec{q}) \propto e^{-U(\vec{q})/k_BT}$, where $U$ is the total
energy of the system (including both potential and kinetic energies).
Note that this the probability distribution leaves out the constant
...
distribution. For example, the expectaton function of the energy is
...
aproximations. So it is clear how much easier it might be to compute
...
Let's say we have some probablity function $p_A(\vec{q})$ that we know, up to some constant. This is of
...
distribtion $p_B(\vec{q})$, defined on the same coordinates $\vec{q}$,
we have:
  \begin{eqnarray*}
...
lot of difficulties in doing this caculation accurately and reliably.
...
states $A$ and $B$, then we have good phase space overap, all of the
...
where $d\vec{r}_A$ are the positions of the solute, and $d\vec{r}_v$ the positions of all solvent molecules, and  and $\Delta u =  u(\vec{r}_A , \vec{r}_v) -  [ u(\vec{r}_v) + u(\vec{r}_A ) ]$ the change in the system's potential energy for coupling the solute to the solvent. $\langle \ldots \rangle_0$ is this case is an average in the \textit{decoupled} ensemble, where the probability disribution we are using is the Boltzmann distribution for the a box containing the solvent and solute, but where there is no interaction between the two. 
...
\end{equation}
where $-\Delta u = [ u(\vec{r}_v) + u(\vec{r}_A ) ] - u(\vec{r}_A , \vec{r}_v)$ is a \textit{decoupling energy}, as it representes the energy of decouplinb or turning off the energy between the solute and the solvent.
...
of expectation values.  Once we have defined $u(\vec{q},\lambda)$, as
...
so of algebra and statistical mechancis shows that, if $\lambda=0$
corresponds to state $B$ and $\lambda=1$ correspond to state $A$,
then:
...
and then integrate this expecation over all values of $\lambda$.
...
Instead, we take advantage of fact that the free energy is a state function and break up the transformation into a series of manageable steps, distributed along a pathway where we gradually morph the system from initial to final state of the desired transformation. Each intermediate state models a molecule that is neither $A$ nor $B$ but an interpolation of the two.
...
In theory, any pathway connecting the two states will work, because free energy is a state variable.  However, many instictively obvious ways to interpret between molecules end up being very noisy and inefficient, but a number of good ways to choose pathway have been developed, discussed in more detail in Sec.~\ref{sec:important_pathway}.

Intermediate alchemical states are defined in terms of one or more alchemical progress parameters $\lambda$ that change the potential energy function $U(\vec{q};\lambda)$ such that the potential energies of the initial and final states are recovered at two particular values $\lambda_0$ and $\lambda_1$. We most often parameterize this path with a single numbe ranging from $\lambda=0$ to $\lambda=1$, with $\lambda$ a single scalar.  However, we could define a path that has two different values, such as separate $\lambda$s for the Coulomb, van der Waals, and bonded terms~\cite{naden}.  Usually, we will treat $\lambda$ as a single scalar, with $\lambda=0$ and $\lambda=1$ as endpoints, as most calculations use this; we will point out where appropriate how the math and treatment are different for a vector of $\lambda$ variables.  
...
and adding together the free energies from an infinte number of simulations that are infinitely closely spaced together. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\section{Choices you need to make when setting up free enegy calculations}
...
All of the above considerations in Section \ref{subsec:hydration} on preparing solvation free energy and partition coefficient calculations also apply to binding free energy calculations, with added complications that protein-ligand binding systems can have significantly slower configuratonal sampling.  This is true for both relative and absolute binding free energies.
...
A third approach borrows ideas from simulated tempering~\cite{marinari1992simulated}. In this scheme, a single replica rapidly explores all of $\lambda$ space by switching between different intermediate $\lambda$ values, as seen in Fig.~\ref{fig:fig_sampling_scheme}\textbf{C}.  This is referred to as expanded ensemble, extended ensemble or even just simulated tempering (although the temperature is not changed when applied to $\lambda$ changes in alchemical free energy calculations).~\cite{lyubartsev1992new, li2007simulated, tan2017optimally} and is supported OpenMM Tools~\cite{andrearizzi2019choderalab}, GROMACS and AMBER TI~\cite{Lee_JPhysChemB_2025_v129_p13063}.    One can show that the expanded ensemble calculations will move back and forth between replicas faster than replica exchange.~\cite{zhang2008comparison,hsu2024replica}  
...
\end{equation}
As descussed earlier, alchemical solvation free energy calculations implement Eq.~(\ref{eq:zratio-solv-dgexc}), by means of \emph{coupling} or \emph{decoupling} computational protocols and alchemical intermediate states as discussed in section \ref{subsec:numerical-implementations}.
