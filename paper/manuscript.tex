...
$q'_s$ --- molecular partition function of the solute at rest \\
$\mathbf{r}_A$, $\mathbf{r}_v$ --- positions of the solute and of the solvent molecules\\
$s$, $v$ --- subscripts labelling the solute and the solvent; in Sec.~\ref{sec:theory-partitioning} the two solvents are $v_1$ and $v_2$. Not to be confused with the reduced potential $u$ or the reduced work $w$ below\\
...
$w(b, \lambda)$ --- perturbation energy function \\
...
$\lambda_i$, $\Lambda_s$ --- atomic and molecular thermal de Broglie wavelengths \\
...
\end{equation}
over the configurational space of the environment ($\Gamma_{\mathrm{env}}$).  For simplicity in this paper, we will usually use the more specific $\beta U(\mathbf{q})$ instead of $u(q\mathbf{q})$, but for more complicated situations where one is dealing with simulations at multiple temperatures or pressures, it can prove useful to write equations in terms of the reduced potential. 
...
We then introduce some number of intermediate alchemical states at values $\lambda_A = \lambda_0, \lambda_1, \cdots, \lambda_K = \lambda_B$ so that each pair of consecutive states $\lambda_k, \lambda_{k+1}$ do share good phase space overlap, meaning, as described above, that there are decent number of samples $\mathbf{q}$ that have moderate probability in both neighboring alchemical states.  Then for $K$ intermediate states (written here for the Gibbs free energy, but true for the free energy of any other ensemble), we get

\begin{equation}
    \Delta G = k_BT \Delta f = k_BT \left(\sum_{k=0}^{K-1} \Delta G_{k,k+1}\right)
\end{equation}
where
\begin{equation}
    \Delta G_{k,k+1} = G_{k+1} - G_{k} = -\ln\frac{Z(\lambda_{k+1})}{Z(\lambda_{k})}
\end{equation}
...
The free energy difference $\Delta F_{01}$ between an initial state 0 and a final state 1 in a canonical ensemble defined by two Hamiltonians $H_0(\mathbf{q},\mathbf{p})$ and $H_1(\mathbf{q},\mathbf{p})$ can be calculated as:
\begin{eqnarray}
\Delta F_{01}  =  -\beta^{-1}\ln \expect{e^{-\beta (H_1(\mathbf{q},\mathbf{p}) - H_0(\mathbf{q},\mathbf{p}))}}_0 =  -\beta^{-1}\ln \expect{e^{-\beta \Delta H(\mathbf{q},\mathbf{p})} }_0
\end{eqnarray}
...
The work of an alchemical transition can be calculated by integrating $\partial H(\mathbf{q},\mathbf{p})/\partial(\lambda)$ over the transition path:

\begin{equation}
W   = \int_{0}^{1} \frac{\partial H(\mathbf{q},\mathbf{p})}{\partial\lambda} d\lambda.  
...
\begin{eqnarray}
 \sum_{k=1}^{n_i} \frac{1}{1 + \exp[\ln(\frac{n_i}{n_j}) + \beta \Delta U_{ij}(\mathbf{q}_k) - \beta \Delta F_{ij})
 ]} \nonumber \\
 =\sum_{l=1}^{n_j} \frac{1}{1 + \exp[\ln(\frac{n_j}{n_i}) - \beta \Delta U_{ij}(\mathbf{q}_l) + \beta \Delta F_{ij})]},
...
Replacing $U_{ij}(\mathbf{q})$ in equation~\ref{eq:bar} by work values $W_k$ and $W_l$ corresponding to the work observed for the forward and reverse transitions, BAR can be written as follows~\cite{shirts2003equilibrium}:
...
\begin{equation}
\frac{df}{d\lambda} = \frac{d}{d\lambda} \left[-\ln \int \frac{\exp^{-\beta U(\lambda,\mathbf{q})}}{Z(\lambda)} d\mathbf{q}\right] = \expect{\frac{dU(\lambda,\mathbf{q})}{d\lambda}\
}_{\lambda} .
\end{equation}
...
\expect{\frac{dU(\lambda,\mathbf{q})}{d\lambda}}_{k}, \] where the weights $w_k$ correspond to a particular choice of numerical integration.
...
Uncertainty estimate is both one of the most confusing parts of any simulation calculation, and one of the most crucial, and so deserves more explanation.  We first outline the basic principles of uncertainty estimation, in a framework based on Markov process theory. In all of these ways of estimating uncertainty, we assume that the underlying dynamics are sufficiently ``mixing'', which roughly means the simulation explores configuration space effectively. As has been widely noted in the literature [\textbf{cite}], this assumption is not always satisfied in practice. 
...
This section describes the simplest and, in many cases, the most robust procedure for estimating uncertainties in free energy calculations. For most practical applications, we recommend this approach when reporting statistical error bars. The procedure consists of simulating an ensemble $\{X^{(1)},\ldots,X^{(N_{\mathrm{rep}})}\}$ of $N_{\mathrm{rep}}$ molecular dynamics trajectories initialized at $\mathbf{q}_0$, and using the resulting free energy estimates $\{\Delta G^{(1)},\ldots,\Delta G^{(N_{\mathrm{rep}})}\}$ to quantify uncertainty. The trajectories $\Delta
G^{(i)} (\cdot)$ are identically distributed and conditionally independent given $\mathbf{q}_0$. For any time $\mathcal{T} > 0$, the variance of any $\Delta G^{(i)} (\mathcal{T})$ can be estimated with the sample variance $S^2 (\mathcal{T})$,
\begin{equation}\label{eq:sample-variance-definition}
  S^2 (\mathcal{T}) = \frac{1}{N_{\mathrm{rep}} - 1} \sum_{i = 1}^{N_{\mathrm{rep}}} (\Delta G^{(i)} (\mathcal{T}) -
  \overline{\Delta G (\mathcal{T})})^2, \qquad \overline{\Delta G (\mathcal{T})} = \frac{1}{N_{\mathrm{rep}}}
  \sum_{i = 1}^{N_{\mathrm{rep}}} \Delta G^{(i)} (\mathcal{T}) .
\end{equation}
In the notation of Eq.~\ref{eqn:empirical-average}, $Y_i = \Delta G^{(i)}(\mathcal{T})$; conditioned on $\mathbf{q}_0$, the $Y_i$ are independent and identically distributed and are trivially a Markov process, with stationary distribution given by the distribution of $\Delta G^{(1)}(\mathcal{T})$. By direct calculation, we have (conditionally on $\mathbf{q}_0$)
\begin{equation}
    \mathbb{E}_{\mathbf{q}_0}[S^2(\mathcal{T})] = \mathrm{Var}_{\mathbf{q}_0}(\Delta G^{(1)}(\mathcal{T}))
\end{equation}
The above equation says that $S^2(\mathcal{T})$ is an unbiased estimator of the variance of $\Delta G^{(1)}(\mathcal{T})$, conditioned on $\mathbf{q}_0$. By the Law of Large Numbers, as $N_{\mathrm{rep}} \rightarrow \infty$, we have $S^2 (\mathcal{T}) \rightarrow \mathrm{Var} _{\mathbf{q}_0}(\Delta
G^{(1)} (\mathcal{T})) = \sigma^2 (\mathcal{T} ; \mathbf{q}_0)$. Furthermore, by conditional independence of the $\Delta G^{(i)}(\mathcal{T})$, for sufficiently large $N_{\mathrm{rep}}$ and any $\mathcal{T}>0$ for which the variance is finite, we have
\begin{equation}\label{eqn:variance-of-empirical-mean}
\mathrm{Var}_{\mathbf{q}_0}(\overline{\Delta G(\mathcal{T})}) = \frac{\mathrm{Var}_{\mathbf{q}_0}(\Delta G^{(1)}(\mathcal{T}))}{N_{\mathrm{rep}}} \approx \frac{S^2(\mathcal{T})}{N_{\mathrm{rep}}}
\end{equation}
...
\end{equation}
which is the gold standard for reporting the error of a free energy estimate $\overline{\Delta G (\mathcal{T})} $ when a single initial condition $\mathbf{q}_0$ is used. Note that $\mathrm{SE}
(\mathcal{T} ; \mathbf{q}_0) \rightarrow 0$ as $N_{\mathrm{rep}} \rightarrow \infty$ so long as $\mathrm{Var}_{\mathbf{q}_0}(\Delta G^{(1)}(\mathcal{T}))$ is finite, which is true if the $\Delta G^{(1)}(\mathcal{T})$ are bounded. Thus the variance can be made arbitrarily small simply by running enough replicates, but we note critically this does \textit{not} mean that we will have estimated the free energy to arbitrary precision: $\Delta G^{(1)}(\mathcal{T})$ in general will not be normally distributed, its value may vary greatly with both $\mathcal{T}$ and $\mathbf{q}_0$, and the variance may not be a good statistic to capture the error, as the distribution may be multi-modal, heavy-tailed and be sensitive to rare events. For example, one replicate out of many might explore a different mode of an important torsional angle, leading to a drastically different free energy estimate. 
...
In the preceding case, we used minimal assumptions on the distribution of $\Delta G^{(i)}(\mathcal{T})$ to obtain estimates on $\sigma^2(\mathcal{T};\mathbf{q}_0)$ by simulating conditionally independent replicates and using the sample variance Eq.~\ref{eq:sample-variance-definition}. To obtain per-replicate (i.e., per-$i$) error estimates, we impose much stricter assumptions on the distribution of $\Delta G^{(i)}(\mathcal{T})$,  namely that they satisfy a Central Limit Theorem (CLT) as $\mathcal{T}\rightarrow \infty$, for each $i=1,\dots,N_{\mathrm{rep}}$:
$$ \sqrt{\mathcal{T}} ( \Delta G^{(i)}(\mathcal{T}) - \Delta G^\star) \Rightarrow N(0, \sigma^2_{\mathrm{asym}}(\mathbf{q}_0))$$
where $\Delta G^\star$ denotes the true free energy difference, $\sigma_{\mathrm{asym}}^2(\mathbf{q}_0)>0$ denotes the so-called "asymptotic variance", $ N(0, \sigma^2_{\mathrm{asym}}(\mathbf{q}_0))$ is a Gaussian distribution with mean 0 and variance $\sigma_{\mathrm{asym}}^2(\mathbf{q}_0)>0$, and $\Rightarrow$ denotes convergence in distribution. %% Should we define this? Might be a little too technical for the audience. Again, Durrett is one of the standard references. 
...
\[
\overline{\Delta G} \equiv \frac{1}{N_{\mathrm{b}}}\sum_{k=0}^{N_{\mathrm{b}}-1} \Delta G_k .
\]
...
\begin{eqnarray}
g &\equiv& 1 + 2 \tau_\mathrm{eq} \approx  \frac{\sigma^2(\mathcal{T};\mathbf{q}_0)}{\sigma_0^2} N \label{eq:statistical-inefficiency-definition} 
\end{eqnarray}
...
\[
E(t)^2 \;\equiv\; \mathrm{Var}_{\mathbf{q}_0}\!\bigl(\Delta G^{(i)}(t)\bigr)
\]
...
% While one might hope for the asymptotic scaling $E(t)\sim t^{-1/2}$, such behavior is rarely observed at practically accessible timescales. Nevertheless, conditioned on their initial states, the estimates $\{\Delta G^{(i)}(t)\}_{i=1}^{N_{\mathrm{rep}}}$ obtained from different replicates are conditionally independent and identically distributed. Consequently, the conditional variance of the averaged estimator
% \[
% \frac{1}{N_{\mathrm{rep}}}\sum_{i=1}^{N_{\mathrm{rep}}} \Delta G^{(i)}(t)
% \]
