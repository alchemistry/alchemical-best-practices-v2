...
function $Z=\int e^{-U(\vec{q})/k_BT}$, so we can then write
...
\frac{p_A(\vec{q})}{Z_A} = 1$ We are making this general, so I won't
define $p(x)$ just yet.  I can always divide and multiply by one, so
we have.

  \begin{eqnarray*}
    1 &=& \int \frac{p_A(\vec{q})}{Z_A} d\vec{q} \\
      &=& \int p_B(\vec{q})/Z_B \frac{p_A(\vec{q})/Z_A}{p_B(\vec{q})/Z_B} d\vec{q} \\
   \langle 1 \rangle_B &=& \langle \frac{p_A(\vec{q})/Z_A}{p_B(\vec{q})/Z_B} \rangle_B \\
           1  &=& \frac{Z_B}{Z_A} \langle \frac{p_A(\vec{q})}{p_B(\vec{q})} \rangle_B \\
           \frac{Z_A}{Z_B}  &=& \langle \frac{p_A(\vec{q})}{p_B(\vec{q})} \rangle_B \\
  \end{eqnarray*}

  We take advantage of the fact that the expectation value of a
  constant is just that constant.  1 is just 1, and $Z_A$ and $Z_B$
  don't depend on $q$ either. We do have to make sure $p_B(\vec{q})$
  ...
  \begin{eqnarray}  
    \frac{Z_A}{Z_B}  &=& \langle \frac{p_A(\vec{q})}{p_B(\vec{q})} \rangle_B \nonumber \\
    \frac{e^{-f_A}}{e^{-f_B}}  &=& \langle \frac{ e^{-u_A(\vec{q})}}{ e^{-u_B(\vec{q}}} \rangle_B \nonumber \\
      e^{-(f_A-f_B)}  &=& \langle e^{-(u_A(\vec{q})-u_B(\vec{q}))}\rangle_B \nonumber \\
      e^{-\Delta f}  &=& \langle e^{-\Delta u(\vec{q}))}\rangle_B \nonumber \\
    \Delta f  &=& - \ln \langle e^{-\Delta u(\vec{q}))}\rangle_B \label{eq:zwanzig}
  \end{eqnarray}

Where $\Delta f = f_A - f_B$ and $\Delta U(\vec{q}) =
u_A(\vec{q})-u_B(\vec{q})$.  This equation is often known as the
Zwanzig equation~\cite{zwanzig}, or exponential averaging, since we
exponentiate our $\delta u$, then average.
...
The key to this working well is that we need $\delta_u(\vec{q}$ not to
vary too much during the simulation.  If it does, then it means some
values of $\vec{q}$ are much more probable in one state than the
other, and simulations of $A$ and $B$ don't really visit the same
configurations at all.  This is frequently called poor phase space
overlap, or poor configuration space overlap.  In that case, just a
few simulation points contribute to the average and it will be very
noisy. If on the other hand, $\vec{q}$ has similar probability in
...
\paragraph{Thermodynamic integration shows }

The Z

Once we have defined $u(\vec{q},\lambda)$, as a function of $\lambda$,
then we can compute the derivative $\frac{du}{d\lambda}$. Although we
will not prove it here, a page or so of algebra and statistical
mechancis shows that, if $\lambda=0$ corresponds to state $B$ and
$\lambda=1$ correspond to state $A$, then:

\begin{eqnarray}
  \delta f = \int_0^1 \left \langle \frac{du}{d\lambda} \right \langle \label{eq:TI_defn}
\end{eqnarray}
...
section~\ref{sec:???}. But yet, again we have turned a calculation of
...
As discussed below, because of poor ensemble overlap, in most cases of interest, alchemical free energy calculations cannot be carried out directly by ensemble averages collected just at the reference or target states. Instead, in a process sometimes called stratification,~\cite{chipot2007free} we take advantage of fact that the free energy is a state function and break up the transformation into a series of manageable steps, distributed along a pathway where we gradually morph the system from initial to final state of the desired transformation (see Section \ref{sec:important_path}). Intermediate alchemical states are typically defined in terms of one or more alchemical progress parameters $\lambda$ controlling the potential energy function $U(\vec{q};\lambda)$ such that the potential energies of the initial and final states are recovered at two particular values $\lambda_0$ and $\lambda_1$. We most often parameterize this path with a single numbe ranging from $\lambda=0$ to $\lambda=1$, with $\lambda$ a single scalar.  However, we could define a path that has two different values, such as separate $\lambda$s for the Coulomb, van der Waals, and bonded terms~\cite{naden}.  Usually, we will treat $\lambda$ as a single scalar, as most calculations use this; we will point out where appropriate how the math and treatment are different for a vector of $\lambda$ variables.  
...
While it is theoretically possible to estimate the ratio of partition functions from samples collected only at states $\lambda_A$ and $\lambda_B$, the efficiency of the free energy estimators rapidly decreases as the phase-space overlap between the two states also decreases~\cite{wu2005phasespace, wu2005phasespacea}.
...
\begin{equation}
    \Delta G_{\mathrm{env}} = k_BT \sum_{k=0}^{K-1} \Delta f(\lambda_k, \lambda_{k+1}),
\end{equation}
from samples collected at all the alchemical states $\{\lambda_k \}$, where $\Delta f$ is the \emph{unitless free energy difference}
\begin{equation}
    \Delta f(\lambda_k, \lambda_{k+1}) = f(\lambda_{k+1}) - f(\lambda_k) = - \ln \frac{Z(\lambda_{k+1})}{Z(\lambda_k)} \, .
\end{equation}
While this strategy usually results in sampling thermodynamic states whose Boltzmann distributions are very similar, thus collecting information that is to some degree redundant, some estimators, such as the Multistate Bennett acceptance ratio (MBAR)~\cite{shirts2008statisticallya}, can exploit similarities between states to improve the precision of the estimates. This is achieved by using the configurations sampled at all alchemical states $\{\lambda_k \}$ to compute the free energy difference $\Delta f(\lambda_i, \lambda_{j})$ between any pair of states $i,j$ (see Sec.~\ref{subsec:estimators}).
