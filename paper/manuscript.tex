...
$p(\vec{q}) \propto e^{-U(\vec{q})/k_BT}$ (or equivalently $e^{-\beta U(\vec{q})}$), where $U$ is the total
...
function $Z=\int e^{-U(\vec{q})/k_BT} d\vec{q}$, so we can then write
$p(\vec{x}) = \frac{e^{-U(\vec{q})/k_BT}}{Z}$.

We can define the \textit{expectation value} of some quantity like
energy or volume with respect to a probability distribution $p(q)$ as
...
$\langle U \rangle_p = \frac{1}{N}\sum_{n=1}^N U(\vec{q}_n)$, as $N$ gets
very large, and the $x_n$'s are generated from the distribution
$p(x)$. But in the limit of enough samples, this will also eventually
become equal to $\int p(\vec{q}) U(\vec{q})d\vec{q}$.  Now, for a box
...
u(\vec{q};\lambda) \equiv \beta \left[ U(\vec{q};\lambda) + p \, V(\vec{q}) + \sum_i \mu_i \, N_i(\vec{q}) + \cdots \right] \, .
\end{equation}
...
\begin{equation}\label{eq:delta-G-env}
    \Delta G_{\mathrm{env}} = - k_BT \ln \frac{Z(\lambda_B)}{Z(\lambda_A)} = - k_BT \ln \frac{\int_{\Gamma_{\mathrm{env}}} \exp\left( -u(\vec{q}; \lambda_B) \right) \, d\vec{q}}{\int_{\Gamma_{\mathrm{env}}} \exp\left( -u(\vec{q}; \lambda_A) \right) \, d\vec{q}} \, ,
\end{equation}
over the configurational space of the environment ($\Gamma_{\mathrm{env}}$).  For simplicity in this paper, we will usually use the more specific $\beta U(\vec{q})$ instead of $u(q\vec{q})$, but for more complicated situations where one is dealing with simulations at multiple temperatures or pressures, it can prove useful to write equations in terms of the reduced potential. 
...
Let's say we have some probability function $p_A(\vec{q})$ that we know, up to some constant. This is of
course the situation we are usually in with molecular simulations, since we know that $p_A \propto e^{-U(\vec{q})}$, but we don't know the partition function $Z_A$.

We have by the definition of the partition function that $\int
Z_A^{-1} p_A(\vec{q}) d\vec{q} = 1$. We are starting by making this
...
distribution $p_B(\vec{q})$, defined on the same coordinates $\vec{q}$,
we have:
  \begin{eqnarray*}
    1 &=& \int \frac{p_A(\vec{q})}{Z_A} d\vec{q} \\
      &=& \int p_A(\vec{q})/Z_A \frac{p_B(\vec{q})/Z_B}{p_B(\vec{q})/Z_B} d\vec{q} \\
    &=& \int p_B(\vec{q})/Z_B \frac{p_A(\vec{q})/Z_A}{p_B(\vec{q})/Z_B} d\vec{q} \\
    \left \langle 1 \right \rangle_B &=& \left \langle \frac{p_A(\vec{q})/Z_A}{p_B(\vec{q})/Z_B} \right \rangle_B \\
           1  &=& \frac{Z_B}{Z_A} \left \langle \frac{p_A(\vec{q})}{p_B(\vec{q})} \right \rangle_B \\
           \frac{Z_A}{Z_B}  &=& \left \langle \frac{p_A(\vec{q})}{p_B(\vec{q})} \right \rangle_B \\
  \end{eqnarray*}
...
don't depend on $\vec{q}$ either. We do have to make sure $p_B(\vec{q})$
isn't zero anywhere, which usually isn't a problem, but can crop up
occasionally if the energy goes to infinity.

  At this point, we now say that $p_A(\vec{q}) = e^{-u_A(\vec{q})}$,
  and $p_B(\vec{q}) = e^{-u_B(\vec{q})}$, meaning that $Z_A = e^{-f_A}$
  ...
  \begin{eqnarray}  
    \frac{Z_A}{Z_B}  &=& \left \langle \frac{p_A(\vec{q})}{p_B(\vec{q})} \right \rangle_B \nonumber \\
    \frac{e^{-f_A}}{e^{-f_B}}  &=& \left \langle \frac{ e^{-u_A(\vec{q})}}{ e^{-u_B(\vec{q}}} \right\rangle_B \nonumber \\
      e^{-(f_A-f_B)}  &=& \left \langle e^{-(u_A(\vec{q})-u_B(\vec{q}))}\right\rangle_B \nonumber \\
      e^{-\Delta f}  &=& \left \langle e^{-\Delta u(\vec{q}))}\right\rangle_B \nonumber \\
    \Delta f  &=& - \ln \left \langle e^{-\Delta u(\vec{q}))}\right\rangle_B \label{eq:zwanzig}
  \end{eqnarray}

Where $\Delta f = f_A - f_B$ and $\Delta u(\vec{q}) =
u_A(\vec{q})-u_B(\vec{q})$.  This equation is often known as the
...
The key to this working well is that we need $\Delta u(\vec{q})$ not
to vary too much during the simulation.  If it does vary a lot, then
it means some values of $\vec{q}$ are much more probable in one state
...
very noisy. If on the other hand, $\vec{q}$ has similar probability in
...
\begin{eqnarray}
  ...
  \frac{
    \int d\vec{r}_A  d\vec{r}_v  e^{-u(\vec{r}_A, \vec{r}_v) } 
  }{
    \int d\vec{r}_A d\vec{r}_v e^{-[u(\vec{r}_v) + u( \vec{r}_A  )] } 
  } \\
&=&  \frac{
    \int d\vec{r}_A  d\vec{r}_v  e^{-\{ u(\vec{r}_A, \vec{r}_v) -  [ u(\vec{r}_v) + u(\vec{r}_A) ] \}} e^{-[u(\vec{r}_v) + u(  \vec{r}_A  )] } 
  }{
    \int d\vec{r}_A d\vec{r}_v e^{-[u(\vec{r}_v) + u( \vec{r}_A  )] } 
  } \\
  &=& \langle e^{-\Delta u(\vec{r}_A, \vec{r}_v)} \rangle_0
  \label{eq:zratio-solv-average}
\end{eqnarray}
where $d\vec{r}_A$ are the positions of the solute, and $d\vec{r}_v$ the positions of all solvent molecules, and $\Delta u =  u(\vec{r}_A , \vec{r}_v) -  [ u(\vec{r}_v) + u(\vec{r}_A ) ]$ the change in the system's potential energy for coupling the solute to the solvent. $\langle \ldots \rangle_0$ is this case is an average in the \textit{decoupled} ensemble, where the probability distribution we are using is the Boltzmann distribution for a box containing the solvent and solute, but where there is no interaction between the two. 
...
\end{equation}
where $-\Delta u = [ u(\vec{r}_v) + u(\vec{r}_A ) ] - u(\vec{r}_A , \vec{r}_v)$ is a \textit{decoupling energy}, as it represents the energy of decoupling or turning off the energy between the solute and the solvent.
...
expectation values.  Once we have defined $U(\vec{q},\lambda)$ (again using the reduced potential to keep it general over different ensembles), as
a function of $\lambda$, then we can compute the derivative
$\frac{du(\vec{q})}{d\lambda}$. Although we will not prove it here, a page or
so of algebra and statistical mechanics shows that, if $\lambda=0$
corresponds to state $B$ and $\lambda=1$ corresponds to state $A$,
then:

\begin{eqnarray}
  \Delta f = \int_0^1 \left \langle \frac{du(\vec{q})}{d\lambda} \right \rangle d\lambda \label{eq:TI_defn}
\end{eqnarray}
...
In the case of a relative binding free energy calculation, for example, this might be achieved by simulating a ``chimeric'' molecule composed of enough atoms to represent both $A$ and $B$. A subset of the energetic terms in $U(\vec{q};\lambda)$ is then modulated by $\lambda$ so that at $\lambda_A$, the atoms that form molecule $A$ are activated and those belonging exclusively to $B$ are non-interacting ``dummy atoms'', while the opposite occurs at $\lambda_B$ (see Sec.~\ref{sec:relative-fe-protocol} and Fig.~\ref{fig:fig_topology} for details).
...
\begin{equation}
U(\vec{q},\lambda) = (1-\lambda) U_0(\vec{q}) + \lambda U_1(\vec{q}), \end{equation}
...
\begin{itemize}
    \item Thermodynamic integration (TI) requires $\frac{\partial U(\vec{q})}{\partial\lambda}$.
...
\begin{eqnarray}
\Delta F_{01}  =  -\beta^{-1}\ln \expect{e^{-\beta (H_1(\vec{q},\vec{p}) - H_0(\vec{q},\vec{p}))}}_0 =  -\beta^{-1}\ln \expect{e^{-\beta \Delta H(\vec{q},\vec{p})} }_0
\end{eqnarray}
...
\begin{eqnarray}
\Delta F_{01}  =   -\beta^{-1}\ln \expect{e^{-\beta \Delta U(\vec{q})} }_0
...
Although the Zwanzig equation is formally correct as long as the two states considered sample the same phase space volume, which is true for standard molecular models, it has some very important numerical issues that mean that it often performs badly for standard free energy calculations, even for small molecules~\cite{shirts2005comparison,lu2003appropriate}. One can show that if the standard deviation of the difference $\Delta U(\vec{q}) = U_1(\vec{q})-U_2(\vec{q})$ over all sampled $\vec{q}$ is large, which in this case, means only several times $k_BT$, then very few samples contribute to the average, and the answer will be both biased and extremely noisy~\cite{lelievre2010free}. Essentially, the method is dominated by contributions of rare snapshots~\cite{jarzynski2006rare, wu2005phasespace, wu2005phasespacea}. 
 ...
 As a reminder, this and many of these formulas can be generalized to other ensembles with the use of the reduced energy $u$.  In the case of NVT (canonical) sampling and assuming the masses do not change, then $u$ is simply $\beta U = U(\vec{q},{p})/k_BT$, and $f$ is $G/k_BT$, but it can be generalized to other ensembles with the proper definition of $f$ and $u$.  We will generally use the canonical formalism in this paper. Technically, for free energy calculations at constant pressure, one should use differences in the enthalpy $U+PV$, but at at constant pressure of 1 atm, the contribution of $P\Delta V$ is nearly zero, perhaps a few hundredths of cal/mol.  
...
The work of an alchemical transition can be calculated by integrating $\partial H(\vec{q},\vec{p})/\partial(\lambda)$ over the transition path:

\begin{equation}
W   = \int_{0}^{1} \frac{\partial H(\vec{q},\vec{p})}{\partial\lambda} d\lambda.  
...
Replacing $U_{ij}(\vec{q})$ in equation~\ref{eq:bar} by work values $W_k$ and $W_l$ corresponding to the work observed for the forward and reverse transitions, BAR can be written as follows~\cite{shirts2003equilibrium}:
...
\begin{equation}
\frac{df}{d\lambda} = \frac{d}{d\lambda} \left[-\ln \int \frac{\exp^{-\beta U(\lambda,\vec{q})}}{Z(\lambda)} d\vec{q}\right] = \expect{\frac{dU(\lambda,\vec{q})}{d\lambda}\
}_{\lambda} .
\end{equation}
...
\begin{equation}
f_i = - \ln \sum_{n=1}^{N} \frac{\exp(-\beta U_i(\vec{q}_n))}{\sum_{k=1}^K N_k \exp(\beta F_k-\beta U_k(\vec{q}_n))},
\end{equation}
...
\end{equation}
In the notation of Eq.~\ref{eqn:empirical-average}, $Z_i = \Delta G^{(i)}(T)$; conditioned on $x_0$, the $Z_i$ are independent and identically distributed and are trivially a Markov process, with stationary distribution given by the distribution of $\Delta G^{(1)}(T)$. By direct calculation, we have (conditionally on $x_0$)
\begin{equation}
...
In the preceding case, we used minimal assumptions on the distribution of $\Delta G^{(i)}(T)$ to obtain estimates on $\sigma^2(T;x_0)$ by simulating conditionally independent replicates and using the sample variance Eq.~\ref{eq:sample-variance-definition}. To obtain per-replicate (i.e., per-$i$) error estimates, we impose much stricter assumptions on the distribution of $\Delta G^{(i)}(T)$,  namely that they satisfy a Central Limit Theorem (CLT) as $T\rightarrow \infty$, for each $i=1,\dots,R$:
