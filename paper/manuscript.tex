...
\begin{itemize}
     \item What are alchemical free energy calculations attempting to do? (Sec.~\ref{subsec:XXXX})
...
Using the same sorts of mathematical tricks, we can actually get ways
to estimate free energies that are more numerically efficient and
robust, such as Bennett's acceptance ratio
(BAR)~\cite{bennett1976efficient,shirts2003equilibrium}) which we
cover in detail in Sec.~\ref{sec:??}.

\paragraph{Thermodynamic integration gives another way to calculate free energies from simulation}

Exponential averaging is not the only way we can get free energies out
