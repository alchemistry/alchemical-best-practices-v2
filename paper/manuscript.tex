...
While solutions to dummy atom challenges may in principle be known, most are not yet implemented in setup tools and free energy pipelines and have not yet been thoroughly characterized, so it seems likely that considerable science and engineering remains to be done in this space. 
 ...
 Absolute binding free energies often suffer from systematic errors arising from insufficient sampling of end states. Using an holo structure as the starting conformation artificially preorganizes the binding site, potentially overestimating binding affinity if the system does not relax to the apo state. This is an area of active research and tentative remedial approaches have been proposed for the error sources identified in ABFE studies. One common source of apo-state undersampling is inadequate water resolvation of the binding pocket. In such cases, a short GCMC like equilibration simulation with water swapping could be performed for each alchemical window to resolvate the pocket to known apo solvation levels~\cite{NithishABFE} ~\cite{BenShalom2020InterfacialWater}. When using such an approach, it is advisable that the GCMC runs be applied per-window in the presence of the partially decoupled ligand to allow for enough configurational overlap across the lambda space\cite{ChenggongHuiGCMC}. For membrane proteins with lipid-exposed binding pockets, slow lipid exchange represents an additional source of apo-state undersampling~\cite{NithishABFE}~\cite{CharlieCookABFE}. In such cases it may be useful to include the free energy of lipid binding as an explicit step in the alchemical cycle. Because this is prohibitively slow at the atomistic level, coarse-grained MD can accelerate the lipid dynamics and identify probable lipid binding sites~\cite{CharlieCookABFE, lopezmartin2025lipidcode}. One must note that mixing free energies derived from models at different levels of atom representation, e.g. Coarse-grained and fully atomistic, may require care to avoid discontinuities in the thermodynamic cycle. Third, and most broadly, protein conformational rearrangement upon ligand removal represents the most common and challenging manifestation of apo state error. Inadequate treatment of such reorganization free energy may render the thermodynamic cycle incomplete and thus bias predictions. One proposed remedy is to seed higher-lambda windows with apo-like structures ~\cite{SchrodingerApoWindow} ~\cite{CharlieCookABFE}, but this approach can produce erroneous affinities due to insufficient configurational overlap between apo-like and holo-like windows~\cite{NithishABFE}. An alternative is to characterize the relevant conformational states using geometric descriptors of the binding pocket \cite{SchrodingerVogele}, then apply enhanced sampling methods to estimate the reorganization free energy ($\Delta G _{holo \rightarrow apo}$) as an explicit thermodynamic contribution to the alchemical cycle ~\cite{NithishABFE}.

A related approach to apo-state conformational change is the non-equilibrium ABFE strategy pioneered by Khalak \textit{et al.}~\cite{khalak2021alchemical}, where separate equilibrium simulations of the apo and holo protein are performed and connected via non-equilibrium switching. This approach allows the long apo equilibration to be seeded directly from experimental coordinates, yielding a more realistic apo ensemble and thereby reducing overestimation of binding free energies. However, the apo-holo sampling problem resurfaces in a different form: when the apo and holo ensembles differ by large protein rearrangements, forward and reverse work distributions overlap poorly, biasing the estimator. As in the equilibrium case, sufficient overlap must be ensured, here between work distributions rather than configurations, which often demands many independent switches or less abrupt switching protocols when large-scale rearrangement is involved.  A pragmatic alternative to the apo-state sampling problem is to apply a reference-compound offset, this is reasonable only if the reorganization free energy is approximately uniform across the ligand series, an assumption that could break down with large conformational differences in the holo state between binders\cite{SchrodingerVogele} ~\cite{Chen2023HitDiscovery} ~\cite{wu2025optimizing}.
