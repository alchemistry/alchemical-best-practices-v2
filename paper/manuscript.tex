...
\author[20]{Nithishwer Mouroug Anand}
...
\orcid{Nithishwer Mouroug Anand}{0000-0003-0852-7141}
...
These calculations use non-physical intermediate states, where the chemical identity of some portion of the system (such as a small molecule ligand or protein side chain) is changed by modifying the potential governing the interactions with the environment for the atoms being modified, inserted, or deleted. The non-physical nature of the transformation is referred to as ``alchemical'', with reference to medieval alchemists who tried to transform one element to another, a term coined by Tembre and McCammon in Ref.~\cite{tembre1984ligandreceptor}.

Fig.~\ref{fig:fig_what_is_alchemy} illustrates common free energy changes that may be difficult to compute with unbiased molecular dynamics methods, but are much more tractable with alchemical methods.
...
\begin{figure*}
    \centering\includegraphics[width=1.0\linewidth]{figures/fig1_what_is_alchemy/calculation_types_and_workflow.png}   
    \caption{\textbf{Illustration of common types of free energies differences that can be calculated using alchemical free energy methods (blue box) and a typical workflow for executing a relative binding free energy calculation (orange box).}
...
% Prerequesites    %
%%%%%%%%%%%%%%%%%%%%
\section{Prerequisites and scope of this best practices guide}
\label{sec:pre}
...
We also assume a basic familiarity with the principles of molecular mechanics, molecular dynamics simulations, statistical mechanics, and the biophysics of protein-ligand association. If you feel unfamiliar with some of these concepts, good starting points can be found in the following references: ~\cite{braun2019best, grossfield2018best, klimovich2015guidelines, shirts2012best}. 
...
\begin{itemize}
     \item What are alchemical free energy calculations attempting to do? (Sec.~\ref{sec:theory})
...
Notably, this document---version 2.0 of the original guide---focuses specifically on best practices on \textit{single} alchemical free energy calculations where the quantity of interest is a single free energy difference (or difference of differences). A companion guide which discusses best practices for running \textit{multiple} alchemical free energy calculations~\cite{mey_largescale_inprep}, covers the considerations involved in high-throughput or large-scale campaigns, such as automation, perturbation network design, and campaign-level data management and analysis. Readers aiming to scale up to many free energy calculations, especially to those form forming a network of related chemical transformations such as found in a lead optimization campaign, will find many important suggestions and ideas relevant to those problems in that document.
...
As some of the theoretical background can be daunting to beginners, we do, however, provide a guide to the essential theory behind alchemical free energy calculations in Sec.~\ref{sec:theory}, with more in-depth derivations in Sec.~\ref{sec:derivations}.
...
\item Free energy estimators that combine both thermodynamic and kinetic data~\cite{wu2016multiensemble, mey2014xtram, wu2014statistically,dtram???}
...
%MRS: this should be reviewed at the end.
\begin{tcolorbox}[title=List of Symbols, colback=green!10!white, breakable]
$A$, $B$ --- generic labels for chemical species and for alchemical end states\\
$a$ --- a generic observable whose average is being estimated\\
$\mathcal{B}$ --- computational budget\\
$C^{\circ}$, $C_s$ --- standard and actual molar solute concentrations \\
$C_i$ --- discrete-time normalized fluctuation auto-correlation function\\
$f$ --- reduced (dimensionless) free energy, defined through $Z = e^{-f}$; $\Delta f$ is a reduced free energy difference, related to the Gibbs free energy by $\Delta G = k_B T \Delta f$\\
...
$\mathcal{H}$ --- reduced (dimensionless) Hamiltonian, $\mathcal{H} \equiv \beta H$\\
$I(\zeta)$ --- binding site indicator function\\
$J(\mathcal{T})$ --- objective quantifying the trade-off between replicate number and trajectory length\\
$k_B$ --- Boltzmann constant \\
$k_H$ --- Henry's constant \\
$K$ --- number of alchemical states\\
$K_b$ --- binding  constant \\
$L$ and $R$ --- generic labels for ligand and receptor\\
$M$ --- batch size; $N_{\mathrm{b}}$ --- number of batches\\
$N$ --- number of samples; $N_{\mathrm{atoms}}$, $N_{\mathrm{rep}}$ --- number of atoms and of replicates\\
$\mathcal{O}$ --- overlap matrix between alchemical states\\
$P^{\circ}$ --- standard pressure \\
$P_{12}$ --- partition coefficient of a solute between the two solvents $v_1$ and $v_2$\\
$p$ --- pressure \\
$\mathbf{q}$ --- a single configuration of the system, i.e. the collection of degrees of freedom that completely specifies it\\
$q'_s$ --- molecular partition function of the solute at rest, the prime denoting that the centre-of-mass translational contribution has been factored out \\
$\mathbf{r}_A$, $\mathbf{r}_v$ --- positions of the solute and of the solvent molecules\\
$s$, $v$ --- subscripts labelling the solute and the solvent (the two solvents of Sec.~\ref{sec:theory-partitioning} being $v_1$, $v_2$); distinct from the reduced $u$ and $w$ below\\
$S^2$ --- sample variance of the free energy estimates across replicates\\
$T$ --- temperature \\
$\mathcal{T}$ --- length (chemical time) of a single trajectory\\
$t_0$ --- equilibration time, i.e. the boundary between the equilibration and production regions \\
$U$ --- potential energy\\
$u$ --- reduced (dimensionless) potential capturing all of the features of a thermodynamic state that change the probability of that state\\
$V_{\rm site}$ --- volume of the binding site\\
$W$, $w$ --- work and reduced (dimensionless) work, $w \equiv \beta W$; $w_{\mathrm{dis}}$ is the reduced dissipated work \\
$x_s$ --- internal coordinates of the solute \\
$Y_i$ --- successive samples of a Markov process\\
$z_s$ --- intramolecular configurational partition function of the solute \\
...
$\lambda$ --- alchemical progress parameter, which may be multidimensional if the system changes along several dimensions \\
$\Lambda_s$ --- molecular thermal de Broglie wavelength of the solute \\
$\mu^\circ$, $\mu$ --- standard and actual solute chemical potential \\
$\pi$ --- stationary (equilibrium) density; $\rho_t$, $\rho_0$ --- transition density at time $t$ and initial density \\
$\sigma_0^2$ --- stationary variance of the observable\\
$\tau_{\mathrm{int}}$ --- integrated auto-correlation time\\
$\Psi_v$ --- solvent-averaged potential of mean force of the solute \\
$\Omega$ --- orientational extent of the binding pose
\end{tcolorbox}
...
% Theory basics    %
%%%%%%%%%%%%%%%%%%%%
\section{Free energy differences are defined using thermodynamics and statistical mechanics}
%\section{Statistical mechanics theory of solvation, solvent partitioning, and binding}
\label{sec:theory}

In this section, we introduce the necessary thermodynamic and statistical mechanics theory of dilute solutions to define the quantities we are trying to calculate using alchemical methods, specifically, solvation, solvent partitioning equilibria and noncovalent molecular binding. The emphasis here is placed on bridging theoretical foundations and intuition. We cover these derivations in more detail in Sec.~\ref{sec:derivations}, as there are important subtleties in the definitions that require very careful analysis.  This section is intended as an overview, and will involve a little bit of handwaving.

\subsection{The chemical potential}
\label{sec:chemical_potential}
Since chemical equilibria are governed by the chemical potentials of the species involved, we begin by considering a statistical mechanics expression for the chemical potential of a solute in a solvent. We will then combine chemical potentials to develop equations for each of the processes.

The chemical potential of a solute $s$ in an ideally diluted solution in a solvent $v$ can be written as~\cite{guggenheim1952mixtures504,gilson1997statisticalthermodynamic,gilson2007calculation}
\begin{equation}
  \mu_s = -k_B T \ln \frac{q'_s}{C_s \Lambda_s^3}
  \label{eq:chemical-potential-definition}
\end{equation}
where $k_B$ is Boltzmann's constant, $T$ is the absolute temperature, $C_s = N_s/V$ is the number concentration of the solute, $\Lambda_s = h/\sqrt{2 \pi M_s k_B T}$ is the thermal de Broglie wavelength of the solute, where $h$ is Planck's constant and $M_s$ is the solute's total mass. 
...
The quantity $q'_s$ in Eq.~\ref{eq:chemical-potential-definition} is the molecular partition function of the solute at rest in the solvent
\begin{equation}
  q'_s = \frac{\Lambda_s^3}{\prod_i \lambda_i^3} 8 \pi^2 z_s
  \label{eq:intra-qpu-def}
\end{equation}
where $\beta = (k_B T)^{-1}$, $\lambda_i$ is the thermal De Broglie wavelength of the $i$-th atom of the solute (not the alchemical parameter as used elsewhere in this document), and $z_s$ is the intramolecular configurational partition function of the solute. See a more complete definition of $z_s$ in Sec.~\ref{sec:derivations}.

Eq.~\ref{eq:chemical-potential-definition} is derived using \textit{classical} statistical mechanics theory~\cite{hill1986statthermo}. Classical mechanics is considered an excellent approximation for the room temperature, aqueous processes studied here, which involve only changes in intermolecular interactions. A quantum-mechanical treatment is necessary for processes not covered here, such as covalent binding, or protonation/deprotonation that involve breaking and forming chemical bonds. At a classical level, equilibrium thermodynamic properties, such as gas solubilities and binding constants, do not depend on atomic masses. Hence, while they are included in Eqs.~\ref{eq:chemical-potential-definition} and~\ref{eq:intra-qpu-def} for completeness, the thermal De Broglie wavelength terms coming from the partition functions of the momenta cancel in the resulting expressions below, and we need not consider them.
...
\begin{equation}
  \mu_s^\circ = -k_B T \ln \frac{q'_s}{C^\circ \Lambda_s^3} \, .
  \label{eq:chemical-potential-standard}
\end{equation}
The standard state of a solution is defined as an ideally dilute solution at concentration $C^\circ$~\cite{levine2009physicalchemistrybook6ed}.

A note on which free energy we are computing. We write the Gibbs free energy $G$ throughout, but a simulation sampling the canonical ensemble strictly yields the Helmholtz free energy, $A$, and the two differ by a $pV$ term. For the processes considered here that term is the pressure-volume work associated with the change in volume of the simulation box between the two end states, which is expected to be negligible at normal pressures~\cite{gilson1997statisticalthermodynamic}. We therefore use $G$ throughout without further comment, and the distinction can be safely ignored unless one is working at elevated pressures or with processes involving large volume changes.

Next, we outline the application of the chemical potential statistical mechanics formalism to describe key biophysical processes: solvation, solvent partitioning, and noncovalent molecular binding.

%In general, translating statistical-mechanics identities into computable quantities via molecular simulations involves expressing ratios of partition functions in terms of ensemble averages. In the present context, the quantity being averaged often turns out to be the exponential of a perturbation energy function (the \textit{Zwanzig relationship}~\cite{zwanzig1954hightemperature}), which is efficiently evaluated using the stratification algorithms and free energy estimators discussed in Sections \ref{subsec:formulations-abfe-rbfe}, \ref{subsec:numerical-implementations},  and \ref{subsec:estimators} rather than directly. In this section, we assume that an implementation is well-formed when expressed as a combination of exponential averages, assuming they can be evaluated in practice.

\subsection{Solvation free energy}\label{sec:theory-solvation}

The standard molar Gibbs free energy of the solvation of a gas species $A$ in a solvent $v$, pictured in Fig.~\ref{fig:fig_what_is_alchemy}, is
\begin{equation}
...
\end{equation}
where we use Eq.~\ref{eq:chemical-potential-standard} for the solution, Eq.~\ref{eq:chemical-potential-definition} for the gas (assumed ideal) at the standard gas concentration $C = P^\circ/k_B T$ where $P^\circ = 1$ bar is the standard pressure, and Eq.~\ref{eq:intra-qpu-def}, noting that all momentum terms cancel. The first term in Eq.~\ref{eq:DG0-solvation-def} is an ideal term $\Delta G^\circ_{\mathrm{slv,ideal}}$ that accounts for the difference in standard states for solution and gases. The second term is the excess solvation free energy $\Delta G_{\mathrm{slv, exc.}}$ that can be evaluated by numerical alchemical calculations as described in this guide. Hence Eq.~\ref{eq:chemical-potential-standard} can be used to compare calculated and experimental standard free energies of solvation~\cite{thompson2004new}.
...
\end{equation}
Eq.~\ref{eq:DG-solvation-excess}, which involves only intramolecular partition functions (as described above),  states that the excess free energy of solvation corresponds to the process of moving the solute from a fixed position and orientation in the gas to a fixed position and orientation in the solvent. However, because in a bulk solvent, the solvent is the same in all translational and rotational directions, this is the same as the free energy of a freely tumbling and moving ligand. This is sometimes referred to as the solvation free energy in the Ben-Naim standard state~\cite{bennaim1984solvation}.  At this point, we pause to see what we have done; we have now expressed the thermodynamic process in terms of ratios of partition functions, which we will be able to estimate using molecular simulations.

\subsection{Free energy of solvent partitioning}\label{sec:theory-partitioning}

The standard molar Gibbs free energy of transfer of a solute species $A$ from a  solvent $v_1$ to a solvent $v_2$, 
\begin{equation}
A(v_1)  \leftrightharpoons A(v_2)
\end{equation}
pictured in Fig.~\ref{fig:fig_what_is_alchemy}, is
\begin{equation}
  \Delta G^\circ_{\mathrm{part}} = -k_B T \ln P_{12} = \mu^\circ_{A(v_2)}  - \mu^\circ_{A(v_1)} = -k_B T \ln \frac{z_{A(v_2)}}{z_{A(v_1)}}
  \label{eq:DG0-partitioning-def}
\end{equation}
where $P_{12} = C_{A(v_2)}/C_{A(v_1)}$ is the partition coefficient of $A$ for the two solvents~\cite{rustenburg2016measuring}. Eq.~\ref{eq:DG0-partitioning-def} 
...
\begin{equation}
  P_{12} = \frac{z_{A(v_2)}}{z_{A(v_1)}} \, .
  \label{eq:partition-coefficient}
\end{equation}
Again, this relation connects calculated ratios of partition functions to experimental partition coefficients. Similarly to Eq.~\ref{eq:DG-solvation-excess}, Eq.~\ref{eq:partition-coefficient} corresponds to the process of transferring the solute from a fixed position and orientation in one solvent to a fixed position and orientation in the other. Again, because in a bulk solvent, the solvent is the same in all translational and rotational directions, this is the same as the free energy of transfer of a freely tumbling and moving ligand.
...
Eq.~\ref{eq:DG0-binding-constant-def} includes the intramolecular configurational partition functions $z_{R(v)}$ and $z_{L(v)}$ for the receptor $R$ and ligand $L$, which are defined in the same way as a ligand alone in solvent. Very importantly, however, the intramolecular configurational partition function $z_{RL(v)}$ has an integration over only a chosen range of configurations in which receptor and ligand are considered ``bound''.

To this end, Gilson et al.~\cite{gilson1997statisticalthermodynamic} introduced an indicator function $I(\zeta)$ equal to one when the six degrees of freedom of the complex that define the position (three degrees of translation) and orientation (three degrees of rotation) of the ligand relative to the receptor, the six of which are collectively denoted as $\zeta$, are within the specified range of a binding site region of volume $V_{\rm site}$ and equal to zero otherwise, so that only the user-defined ``bound'' states count in the partition function for $z_{RL(v)}$.  The value of the binding free energy therefore depends on the chosen structural definition of the bound complex.

The definition of the bound macrostate of the complex is thus \textit{essential} to defining ligand binding.  Unlike bond lengths, bond angles, and dihedral angles, the three degrees of freedom of the complex that correspond to the position of the ligand relative to the receptor are unbounded. Unless we define specific limits to these variables, the bound state of the complex cannot be distinguished from the unbound state, and the free energy difference between the two states will be undefined.  In fact, this approximation may be slightly too simple, as there may be cases where only some configurations of a highly flexible ligand actually have experimental effect, so the indicator function $I$ may need to include configurations of the ligand or even protein as well. 

The definition of macrostates that are \textit{defined by specific choices of the microstates} is required in \textit{many} areas of computational chemistry and biophysics. Examples include needed to specify the range of backbone angles to include in $\alpha$ and $\beta$ structures when calculating  the relative populations of $\alpha$-helical and $\beta$-strand conformations of peptides, or even calculating whether a protein is folded or not. Experimentally, what macrostate a microstates belongs to is that they are different functionally. Only certain configurations have biological or experimental effects, and thus when we analyze any sort of simulation, we should try to figure out what simulation variables correspond to the experimentally configurations that have the measured effect. 

This interpretation of experimental reporters of binding is a complex issue in itself that illustrates this principle. For example, an experimental method that identifies any energetic interaction between a protein and a ligand (such as isothermal titration calorimetry) will indicate a different bound state than one that only detects a ligand that is positioned in a specific location on the protein (such as changes in polarization upon displacement of a probe ligand). The relationship between the predictions of this theory and experimental reporters (thermochemical, spectroscopic, inhibition kinetics, and others) of complex formation has been discussed~\cite{gilson1997statisticalthermodynamic,mihailescu2004theory,gallicchio2011recent,simonson2016physical,gallicchio2021comppeptsci}. All the different definitions of the bound state yield nearly equivalent values for the free energy when binding is strong and specific, and the definition of the bound complex encompasses the major bound poses while excluding obvious unbound configurations. Weaker, less specific binding interactions, as discussed in Sec.~\ref{sec:weak-binders} require somewhat more effort to define, and will often give different results depending on the type of experiment used to measure them.  As mentioned above, in some cases it is valuable to consider specific definitions of the complex, for example, by restricting the ligand's orientation to pinpoint the relative contributions of different binding poses.

Eq.~\ref{eq:DG0-binding-constant-def} is the basis of a variety of alchemical computational implementations that have been developed to study molecular binding by alchemical means. Some of the key implementations, specifically double decoupling and alchemical transfer, are discussed in Sec.~\ref{subsec:formulations-abfe-rbfe}. Generally, to formulate the statistical mechanics expression of the binding constant Eq.~\ref{eq:DG0-binding-constant-def} as an ensemble average, we first need to ensure that the configurational integrals in the numerator and denominator are over the same set of degrees of freedom. Note that the intramolecular configurational partition function of the complex, $z_{RL(v)}$, includes six more degrees of freedom---the position and orientation of the ligand relative to the receptor collectively described by the variable $\zeta$---than the product of the intramolecular partition functions, $z_{R(v)} z_{L(v)}$, of the separated receptor and ligand.


As discussed in Section~\ref{sec:derivations}, we can interpret Eq.~\ref{eq:DG0-binding-constant-def} as: 
\begin{eqnarray}
  \Delta G^\circ_b &=&  - k_B T \ln K_b = - k_B T \ln C^\circ V_{\rm site} - k_B T \ln \frac{z_{RL(v)}}{z_{R(v)} z_{L(v)}} \\
{\rm \ }  &=& \Delta G^\circ_{b,{\rm ideal}} + \Delta G_{b,{\rm exc.}} \label{eq:DG0-def-Vsite}
\end{eqnarray}
The term $\Delta G^\circ_{b,{\rm ideal}} = - k_B T \ln C^\circ V_{\rm site}$ in Eq.~\ref{eq:DG0-def-Vsite}, where $V_{\rm site}$ is the binding site volume,   is interpreted as the \emph{ideal} component of the standard binding free energy, the binding free energy that would be observed in the absence of molecular interactions. Conversely, the term $\Delta G_{b,{\rm exc.}}$ is the \emph{excess} component that depends on the specific molecular composition of the system and on the corresponding interactions. The general expression of the excess binding free energy in terms of partition functions is given in Section~\ref{sec:derivations}. The calculation of the excess component is the target of the alchemical molecular simulation approaches discussed here, in contrast to the ideal term, which is generally evaluated analytically~\cite{boresch2003absolute}.

\subsection{Simulating binding events is computationally expensive}

The appeal of alchemical approaches to study binding can be best appreciated by considering the simple alternative, which is evaluating the binding constant directly from the law of mass action definition
\begin{equation}\label{eq:law-mass-action}
 K_b = C^{\circ}\frac{[RL]}{[L][R]},
\end{equation}
where $[RL]$, $[L]$, and $[R]$ are the equilibrium concentrations of complex, ligand, and receptor. One could do this by, for example, monitoring the relative population $P(RL)/P(R+L)$ of bound and unbound configurations in molecular dynamics simulations~\cite{jong2011determining,pan2017quantitative}. Once we have defined the bound state by defining our indicator
function $I$, then at each time point in the simulation, our ligand is
either in or out, and we can calculate the ratio of concentrations.
...
Equivalently, one could calculate $K_b = \frac{k_{on}}{k_{off}}$. Similarly, because of the long time-scale of transition events and the need to observe many events to extract probabilities, it would be very difficult to estimate solvation free energies and solvent partition coefficients by simulating explicitly the motion of solutes in and out of solutions.

There are also alternatives to alchemical calculations which compute the free energy of binding by building potential of mean force profiles along a reaction coordinate~\cite{woo2005calculation,velez-vega2013overcoming,limongelli2013funnel,heinzelmann2017attachpullrelease}, but these methods require prior knowledge of a high-probability binding pathway, which is not easily available, and are not generally applicable to relative binding (see below), which is especially relevant in the prospective scenarios typical of the drug development process.

\subsection{Absolute versus relative binding free energies}

%MRS: potentially some more information could be added here. 
The free energy derived above in Eq.~\ref{eq:DG0-binding-constant-def} is termed an ``absolute'' free energy.  It is still a free energy difference, as it is the difference between the ligand being in the same box of solvent as the protein, and the ligand and receptor each being in their own solvent box.

However, often what we really need to know is not the free energy of binding, but how much better one ligand is binding than another ligand.  This is really all we need to know in lead optimization, where we seek to find a new compound somewhat different from a reference compound, but that binds more tightly.  Relative free energy calculations avoid the need to simulate binding and unbinding events by making use of the fact that the free energy is a state function and exploiting the thermodynamic cycle illustrated in Fig.~\ref{fig:fig_binding_thermodynamic_cycle}.  Fig.~\ref{fig:fig_binding_thermodynamic_cycle} tells us that the difference in free energy of binding between toluene (B) and benzyl alcohol (A) can be computed by running two independent calculations estimating the free energy cost of mutating A into B in the binding pocket ($\Delta G_{bound}$) and in solvent ($\Delta G_{unbound}$) saving us the need to simulate the physical binding process of the two compounds. A significant amount of the remainder of the paper will specifically look at the different choices that need to be made when electing to calculate either absolute or relative free energies of binding.
...
%MRS: This is what people say,  but  do we have some good citations here? What other factors are there? This is a huge question that we really should advise people on if we feel it is the case.
The general consensus is that relative binding free energies are more accurate ways to calculate, most likely because of various types of cancellation of errors. Absolute free energies involve larger perturbations in the system, as one needs to remove and add more atoms from the system. Additionally, if there are errors in the force fields describing the protein and the ligands, any errors in interactions that are conserved between the two ligands will tend to cancel out in the results.

\subsection{How does one use alchemical transformations to calculate free energy differences?}
\label{subsec:numerical-implementations}
\paragraph{How can we get free energies from molecular simulations?}

When we carry out a properly run molecular dynamics or Monte Carlo
simulation in, for example, the canonical (constant particle number
$N$, constant temperature $T$, constant volume $V$) ensemble, then we
are generating samples consistent with the Boltzmann distribution
$p(\mathbf{q}) \propto e^{-U(\mathbf{q})/k_BT}$ (or equivalently $e^{-\beta U(\mathbf{q})}$), where $U$ is the total
energy of the system (including both potential and kinetic energies).
Note that this probability distribution leaves out the constant
needed to make this a true probability distribution, i.e. one that
integrates to 1, also termed a \textit{normalized} probability
distribution.  That constant of proportionality is the partition
function $Z=\int e^{-U(\mathbf{q})/k_BT} d\mathbf{q}$, so we can then write
$p(\mathbf{q}) = \frac{e^{-U(\mathbf{q})/k_BT}}{Z}$.

We can define the \textit{expectation value} of some quantity like
energy or volume with respect to a probability distribution $p(\mathbf{q})$ as
the average of some quantity from many, many samples from the that
distribution. For example, the expectation function of the energy is
$\langle U \rangle_p = \frac{1}{N}\sum_{n=1}^N U(\mathbf{q}_n)$, as $N$ gets
very large, and the $\mathbf{q}_n$'s are generated from the distribution
$p(\mathbf{q})$. But in the limit of enough samples, this will also eventually
become equal to $\int p(\mathbf{q}) U(\mathbf{q})d\mathbf{q}$.  Now, for a box
of 30,000 atoms, this is a $3\times30,000 = 90,000$ dimensional
integral, which is pretty much impossible to do without severe
approximations. So it is clear how much easier it might be to compute
expectation values of complicated systems with simulations rather than
integrals.  So what we will do is find ways to turn the integrals we
need to carry out to calculate ratio of partition functions into
expectation values.

%One nice thing about expectation values is we don't actually have to
%know the normalizing constant for the probability distribution we are
%carrying out. We will get the same average regardless.  %MRS: clarify

We talked above about using the canonical ensemble, but there are
other ensembles as well. Thermodynamic parameters such as changes in
volume $V$ when simulating at constant pressure $p$ or changes in
number of molecules $N_i$ of species $i$ at constant chemical
potential $\mu_i$ (e.g., number of waters or ions) by introducing the
\textit{reduced potential}~\cite{shirts2008statisticallya}
\begin{equation}\label{eq:reduced-potential}
u(\mathbf{q};\lambda) \equiv \beta \left[ U(\mathbf{q};\lambda) + p \, V(\mathbf{q}) + \sum_i \mu_i \, N_i(\mathbf{q}) + \cdots \right] \, .
\end{equation}
Here, the collection of thermodynamic and alchemical parameters $\{\beta, \lambda, p, \mu, \ldots\}$ defines a \emph{thermodynamic state}.
In the context of alchemical calculations, in which the thermodynamic states vary only in their value of $\lambda$, these are also referred to as \emph{alchemical states}.
The free energy and the work follow the same convention and are likewise dimensionless: the reduced free energy $f$ is defined through $Z = e^{-f}$, and the reduced work of a transformation is $w \equiv \beta W$.
...
\begin{equation}\label{eq:delta-G-env}
    \Delta G_{\mathrm{env}} = - k_BT \ln \frac{Z(\lambda_B)}{Z(\lambda_A)} = - k_BT \ln \frac{\int_{\Gamma_{\mathrm{env}}} \exp\left( -u(\mathbf{q}; \lambda_B) \right) \, d\mathbf{q}}{\int_{\Gamma_{\mathrm{env}}} \exp\left( -u(\mathbf{q}; \lambda_A) \right) \, d\mathbf{q}} \, ,
\end{equation}
over the configurational space of the environment ($\Gamma_{\mathrm{env}}$).  From here on we write the estimators of Sec.~\ref{subsec:estimators} and the uncertainty analysis of Sec.~\ref{subsec:uncertainty} in terms of these reduced quantities. Doing so keeps the results valid in any ensemble, which matters as soon as one deals with simulations at multiple temperatures or pressures, and it removes a good deal of clutter from the equations. A reader working exclusively in the canonical ensemble at a single temperature may simply read $u$ as $\beta U(\mathbf{q})$, $f$ as $\beta G$, and $w$ as $\beta W$ throughout.

While it is generally not feasible to directly compute the two partition functions $Z(\lambda)$, for the same reason we can't easily compute binding free energies with \ref{eq:law-mass-action}, several estimators have been devised to robustly estimate the ratio of partition functions in Eq.~\ref{eq:delta-G-env} (see Sec.~\ref{subsec:estimators}) from a set of configurations usually collected with MD simulations from the thermodynamic states defined at $\lambda_A$ and $\lambda_B$ as well as, importantly, configurations from simulating  intermediates in between these two end states.

\paragraph{An example: the Zwanzig relationship connects ratios of partition functions to expectation values of energy differences}

Let's say we have some probability function $p_A(\mathbf{q})$ that we know, up to some constant. This is of
course the situation we are usually in with molecular simulations, since we know that $p_A \propto e^{-U(\mathbf{q})}$, but we don't know the partition function $Z_A$.

We have by the definition of the partition function that $\int
Z_A^{-1} p_A(\mathbf{q}) d\mathbf{q} = 1$. We are starting by making this
very general, so we won't define $p_A(x)$ just yet. We can always
divide and multiply by one, so if we have some other probability
distribution $p_B(\mathbf{q})$, defined on the same coordinates $\mathbf{q}$,
we have:
  \begin{eqnarray*}
    1 &=& \int \frac{p_A(\mathbf{q})}{Z_A} d\mathbf{q} \\
      &=& \int p_A(\mathbf{q})/Z_A \frac{p_B(\mathbf{q})/Z_B}{p_B(\mathbf{q})/Z_B} d\mathbf{q} \\
    &=& \int p_B(\mathbf{q})/Z_B \frac{p_A(\mathbf{q})/Z_A}{p_B(\mathbf{q})/Z_B} d\mathbf{q} \\
    \left \langle 1 \right \rangle_B &=& \left \langle \frac{p_A(\mathbf{q})/Z_A}{p_B(\mathbf{q})/Z_B} \right \rangle_B \\
           1  &=& \frac{Z_B}{Z_A} \left \langle \frac{p_A(\mathbf{q})}{p_B(\mathbf{q})} \right \rangle_B \\
           \frac{Z_A}{Z_B}  &=& \left \langle \frac{p_A(\mathbf{q})}{p_B(\mathbf{q})} \right \rangle_B \\
  \end{eqnarray*}

We take advantage of the fact that the expectation value of a constant
is just that constant.  The average of 1 is just 1, and $Z_A$ and $Z_B$
don't depend on $\mathbf{q}$ either. We do have to make sure $p_B(\mathbf{q})$
isn't zero anywhere, which usually isn't a problem, but can crop up
occasionally if the energy goes to infinity.

  At this point, we now say that $p_A(\mathbf{q}) = e^{-u_A(\mathbf{q})}$,
  and $p_B(\mathbf{q}) = e^{-u_B(\mathbf{q})}$, meaning that $Z_A = e^{-f_A}$
  and $Z_B = e^{-f_B}$.  We use the reduced energy $u$ and reduced free energy $f$ to make the finding general over any ensemble, but one can always replace it with just $\beta U$ for specificity of the canonical ensemble with a single $T$ if you want.  So we can say that 

  \begin{eqnarray}  
    \frac{Z_A}{Z_B}  &=& \left \langle \frac{p_A(\mathbf{q})}{p_B(\mathbf{q})} \right \rangle_B \nonumber \\
    \frac{e^{-f_A}}{e^{-f_B}}  &=& \left \langle \frac{ e^{-u_A(\mathbf{q})}}{ e^{-u_B(\mathbf{q}}} \right\rangle_B \nonumber \\
      e^{-(f_A-f_B)}  &=& \left \langle e^{-(u_A(\mathbf{q})-u_B(\mathbf{q}))}\right\rangle_B \nonumber \\
      e^{-\Delta f}  &=& \left \langle e^{-\Delta u(\mathbf{q}))}\right\rangle_B \nonumber \\
    \Delta f  &=& - \ln \left \langle e^{-\Delta u(\mathbf{q}))}\right\rangle_B \label{eq:zwanzig}
  \end{eqnarray}

Where $\Delta f = f_A - f_B$ and $\Delta u(\mathbf{q}) =
u_A(\mathbf{q})-u_B(\mathbf{q})$.  This equation is often known as the
Zwanzig relation~\cite{zwanzig1954hightemperature}, or exponential
averaging, since we exponentiate our $\Delta u$, then average, then
take the logarithm.

So we have now changed the problem of calculating the ratio of two
partition functions between states $A$ and $B$ (seemingly impossible
to do by calculus!) to the calculation of the expectation value during
a simulation of state $B$ of the difference of energies between states
$A$ and $B$.  This \textit{seems} manageable, though clearly there's a
lot of difficulties in doing this calculation accurately and reliably.

We didn't make any distinction between $A$ and $B$, so we can just
swap the labels $A$ and $B$, and compute a free energy difference by
doing a simulation of state $A$ as well.  We will get out $-\Delta f =
f_B - f_A$, or minus the free energy we get by doing the simulation
from state $B$.

The key to this working well is that we need $\Delta u(\mathbf{q})$ not
to vary too much during the simulation.  If it does vary a lot, then
it means some values of $\mathbf{q}$ are much more probable in one state
than the other, and simulations of $A$ and $B$ don't really visit the
same configurations at all.  This is frequently called poor phase
space overlap, or poor configuration space overlap.  In that case,
just a few simulation points contribute to the average and it will be
very noisy. If on the other hand, $\mathbf{q}$ has similar probability in
states $A$ and $B$, then we have good phase space overlap, all of the
numbers we average in the expectation value are similar in size, and
the average converges quickly.

In the case of the movement of a ligand from solvent to vacuum, then
the ratio of partition functions $\frac{z_{A(v)}}{z_{A(\mathrm{g})}}$
  is:

\begin{eqnarray}
  \frac{z_{A(v)}}{z_{A(\mathrm{g})}} &=&
  \frac{
    \int d\mathbf{r}_A  d\mathbf{r}_v  e^{-u(\mathbf{r}_A, \mathbf{r}_v) } 
  }{
    \int d\mathbf{r}_A d\mathbf{r}_v e^{-[u(\mathbf{r}_v) + u( \mathbf{r}_A  )] } 
  } \\
&=&  \frac{
    \int d\mathbf{r}_A  d\mathbf{r}_v  e^{-\{ u(\mathbf{r}_A, \mathbf{r}_v) -  [ u(\mathbf{r}_v) + u(\mathbf{r}_A) ] \}} e^{-[u(\mathbf{r}_v) + u(  \mathbf{r}_A  )] } 
  }{
    \int d\mathbf{r}_A d\mathbf{r}_v e^{-[u(\mathbf{r}_v) + u( \mathbf{r}_A  )] } 
  } \\
  &=& \langle e^{-\Delta u(\mathbf{r}_A, \mathbf{r}_v)} \rangle_0
  \label{eq:zratio-solv-average}
\end{eqnarray}
where $d\mathbf{r}_A$ are the positions of the solute, and $d\mathbf{r}_v$ the positions of all solvent molecules, and $\Delta u =  u(\mathbf{r}_A , \mathbf{r}_v) -  [ u(\mathbf{r}_v) + u(\mathbf{r}_A ) ]$ the change in the system's potential energy for coupling the solute to the solvent. $\langle \ldots \rangle_0$ is this case is an average in the \textit{decoupled} ensemble, where the probability distribution we are using is the Boltzmann distribution for a box containing the solvent and solute, but where there is no interaction between the two. 

As above, we can consider the reciprocal of Eq.~\ref{eq:zratio-solv-average} and express it in terms of an ensemble average $\langle \ldots \rangle_1$ in the coupled ensemble:
\begin{equation}
  \frac{z_{A(v)}}{z_{A(\mathrm{g})}} = \frac{1}{\langle e^{\Delta u} \rangle_1}
\end{equation}
where $-\Delta u = [ u(\mathbf{r}_v) + u(\mathbf{r}_A ) ] - u(\mathbf{r}_A , \mathbf{r}_v)$ is a \textit{decoupling energy}, as it represents the energy of decoupling or turning off the energy between the solute and the solvent.
...
Using the same sorts of mathematical tricks, we can estimate
free energies in ways that are more numerically efficient and robust, such as
Bennett's acceptance ratio
(BAR)~\cite{bennett1976efficient,shirts2003equilibrium}) which we
cover in detail in Sec.~\ref{subsec:estimators}.

For transfer of a solvent from one solute to another, we can take
advantage of the fact that if we perform two calculations of the
transfer of solvent to vacuum, then we can reverse one of those
calculations and add them together.  The intermediate vacuum state
cancels out, and we obtain the transfer free energy.  Calculating the
free energy of ligand binding is more complicated, primarily because defining the end states can be very complicated, as discussed in Sec~\ref{sec:non-covalent-molecular-binding}. We cover in Sec.\ref{subsec:formulations-abfe-rbfe} how to set up the proper partition functions for ligand binding in two different ways.


\paragraph{Thermodynamic integration gives another way to calculate free energies from simulation}

Exponential averaging is not the only way we can relate free energies to
expectation values.  Once we have defined $U(\mathbf{q},\lambda)$ (again using the reduced potential to keep it general over different ensembles), as
a function of $\lambda$, then we can compute the derivative
$\frac{du(\mathbf{q})}{d\lambda}$. Although we will not prove it here, a page or
so of algebra and statistical mechanics shows that, if $\lambda=0$
corresponds to state $B$ and $\lambda=1$ corresponds to state $A$,
then:

\begin{eqnarray}
  \Delta f = \int_0^1 \left \langle \frac{du(\mathbf{q})}{d\lambda} \right \rangle d\lambda \label{eq:TI_defn}
\end{eqnarray}
In other words, we keep track of $\frac{du}{d\lambda}$ as we run the
simulation, average that to get an estimate of the expectation value,
and then integrate this expectation over all values of $\lambda$.

If you like playing with Taylor series, you can also show that Eq.~\ref{eq:TI_defn} can be derived by taking the Zwanzig equation,
and adding together the free energies from an infinite number of simulations that are infinitely closely spaced together. 

Clearly we do not want to run simulations at an infinite number of
$\lambda$ values, so we will have to approximate this integral
somehow. We'll discuss how to make that choice in
Sec.~\ref{sec:important_path}. But yet, again we have turned a calculation of
free energies (or equivalently, ratios of partition functions) into
calculations of expectation values of some quantity.

\paragraph{Why do alchemical calculations need unphysical intermediate states?}

While it is theoretically possible to estimate the ratio of partition functions from samples collected only at states $\lambda_A$ and $\lambda_B$, the uncertainty of free energy estimators rapidly increases as the phase-space overlap between the two states decreases~\cite{wu2005phasespace, wu2005phasespacea}. As described above, the phase-space overlap between two thermodynamic states measures the degree to which high-probability configurations (i.e., those with low potential energy) in one state are also high-probability configurations in another state (see Sec.~\ref{sec:are-they-good} and Fig.~\ref{fig:fig_what_is_lambda}). We make ``good overlap'' more quantitative in section~\ref{sec:data_analysis}.

Instead, we take advantage of the fact that the free energy is a state function and break up the transformation into a series of manageable steps, distributed along a pathway where we gradually morph the system from initial to final state of the desired transformation. Each intermediate state models a molecule that is neither $A$ nor $B$ but an interpolation of the two.

In the case of a relative binding free energy calculation, for example, this might be achieved by simulating a ``chimeric'' molecule composed of enough atoms to represent both $A$ and $B$. A subset of the energetic terms in $U(\mathbf{q};\lambda)$ is then modulated by $\lambda$ so that at $\lambda_A$, the atoms that form molecule $A$ are activated and those belonging exclusively to $B$ are non-interacting ``dummy atoms'', while the opposite occurs at $\lambda_B$ (see Sec.~\ref{sec:relative-fe-protocol} and Fig.~\ref{fig:fig_topology} for details).

In theory, any pathway connecting the two states will work, because free energy is a state variable.  However, many instinctively obvious ways to interpolate between molecules end up being very noisy and inefficient, but a number of good ways to choose a pathway have been developed, discussed in more detail in Sec.~\ref{sec:important_path}.

Intermediate alchemical states are defined in terms of one or more alchemical progress parameters $\lambda$ that change the potential energy function $U(\mathbf{q};\lambda)$ such that the potential energies of the initial and final states are recovered at two particular values $\lambda_0$ and $\lambda_1$. We most often parameterize this path with a single number ranging from $\lambda=0$ to $\lambda=1$, with $\lambda$ a single scalar.  However, we could define a path that has two different values, such as separate $\lambda$s for the Coulomb, van der Waals, and bonded terms~\cite{naden}.  Usually, we will treat $\lambda$ as a single scalar, with $\lambda=0$ and $\lambda=1$ as endpoints, as most calculations use this; we will point out where appropriate how the math and treatment are different for a vector of $\lambda$ variables.  
%MRs: talk more about how even if there is a multiple component vector, for TI we take a path.  But with MBAR, we can take all paths simultaneous. 
We then introduce some number of intermediate alchemical states at values $\lambda_A = \lambda_0, \lambda_1, \cdots, \lambda_K = \lambda_B$ so that each pair of consecutive states $\lambda_k, \lambda_{k+1}$ do share good phase space overlap, meaning, as described above, that there are decent number of samples $\mathbf{q}$ that have moderate probability in both neighboring alchemical states.  Then for $K$ intermediate states we get

\begin{equation}
    \Delta f = \sum_{k=0}^{K-1} \Delta f_{k,k+1} \, , \qquad \Delta G = k_BT \, \Delta f
\end{equation}
where
\begin{equation}
    \Delta f_{k,k+1} = f_{k+1} - f_{k} = -\ln\frac{Z(\lambda_{k+1})}{Z(\lambda_{k})}
\end{equation}

Which we can compute from samples collected at all the alchemical states $\{\lambda_k \}$.



This strategy usually results in sampling thermodynamic states whose Boltzmann distributions are very similar, thus collecting information that is to some degree redundant, some estimators, such as the multistate Bennett acceptance ratio (MBAR)~\cite{shirts2008statisticallya}, can exploit similarities between states to improve the precision of the estimates, using \textit{all} of the data at the same time, with all of the configurations sampled at all alchemical states $\{\lambda_k \}$ to compute the free energy difference $\Delta f(\lambda_i, \lambda_{j})$ between any pair of states $i,j$ (see Sec.~\ref{subsec:estimators}).

Finally, non-equilibrium free energy techniques provide an alternate approach to this problem, driving $\lambda$ at states between the endpoints (see Sec.~\ref{sec:sampling_schemes})~\cite{jarzynski1997nonequilibrium,jarzynski1998equilibrium,crooks2000pathensemble, gapsys2020large}. 
...
Alchemical free energy calculations involving small molecules seem to achieve, for common benchmark systems, root mean square (RMS) errors relative to experiment around 1-2 kcal/mol depending on force field, system, and a variety of other factors such as simulation time, sampling method, and whether the calculations employed are absolute or relative~\cite{baumann2025largescale, fuMetaAnalysisRevealsThat2022, hahn2024current, gapsys2020large}. Example datasets and case studies are available in the companion article on best practices for large-scale alchemical free energy calculations~\cite{mey_largescale_inprep}.
...
%MRS: check this - more detail than needed here.
...
% Is this too detailed example for best practices?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\section{Choices you need to make when setting up free energy calculations}
...
Preliminary considerations necessary for using free energy methods to compute partition coefficients are generally more straightforward. For example, a 3-D minimized structure of a solute can be generated with a simple tool such as Open Babel and solvated to prepare the input to compute a free energy of hydration~\cite{oboyle2011open}. Despite their relative simplicity, care should still be taken when preparing these systems to avoid introducing systematic errors.
...
\subsection{Choices for free energies of binding}
\label{subsec:binding}
All of the above considerations in Section \ref{subsec:hydration} on preparing solvation free energy and partition coefficient calculations also apply to binding free energy calculations, with added complications that protein-ligand binding systems can have significantly slower configurational sampling.  This is true for both relative and absolute binding free energies.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Implementation details of alchemical transformations}
\label{sec:simulation_protocol_choice}

\subsection{Alchemical binding free energy formulations}
...
In the alchemical literature, the term ``absolute binding free energy'' is another name for the standard binding free energy $\Delta G^\circ_b = - k_B T \ln K_b$ between a receptor and its ligand.  Here, we discuss two methods, Double-Decoupling (DDM) and Alchemical Transfer (ATM), used to evaluate it by alchemical computer simulations.  In the following analysis, we use the solvent-averaged solute forms, which is defined more explicitly in the later section of the paper Sec.~\ref{sec:derivations}, digging more deeply into the details of the theory to be formally correct.  We will use real potentials and temperatures $U$ and $\beta$, rather than reduced potentials, for concreteness and to make these calculations more easily implementable, and use $G$ for the free energy, though technically one would need to add a (negligible for protein-ligand binding at ambient conditions) $P\Delta V$ term. 
...
Z_{N_v, RL} = \int d\mathbf{r}_v dx_R dx_L d\zeta I(\zeta) e^{-\beta U(x_R, x_L, \zeta, \mathbf{r}_v)}
\end{equation}
...
\begin{equation}
  z_{R(v)} z_{L(v)} =
  \frac{
    \int d\mathbf{r}_v dx_R dx_L d\zeta I^\ast(\zeta) e^{-\beta U(x_R, x_L, \zeta, \mathbf{r}_v)}
  }{
...
\begin{equation}\label{eq:atm-partfunc-ratio}
  K_b = C^\circ V_{\rm site} \frac{
    \int d\mathbf{r}_v dx_R dx_L d\zeta I(\zeta) e^{-\beta U(x_R, x_L, \zeta, \mathbf{r}_v)}
  }{
    \int d\mathbf{r}_v dx_R dx_L d\zeta I^\ast(\zeta) e^{-\beta U(x_R, x_L, \zeta, \mathbf{r}_v)}
  }
\end{equation}
...
\Delta U(x) = U(x_R, x_L, \zeta + \vec{h}, \mathbf{r}_v) - U(x_R, x_L, \zeta, \mathbf{r}_v)
\end{equation}
...
U_{1/2}(x) = \frac12 [ U(x_R, x_L, \zeta + \vec{h}, \mathbf{r}_v) + U(x_R, x_L, \zeta, \mathbf{r}_v)  ] 
\end{equation}
...
\begin{equation}
\Delta U_1(x) = U(x_R, x_L, \zeta + \vec{h}, \mathbf{r}_v) - U_{1/2}(x)
\end{equation}
Setting,
\begin{equation}
...
\begin{equation}\label{eq:atm-partfunc-ratio-rbfe-1}
  \frac{K_b(B)}{K_b(A)} = \frac{
    \int d\mathbf{r}_v dx_R dx_A d\zeta_A dx_B d\zeta_B I^\ast(\zeta_A) I(\zeta_B)  e^{-\beta U(x_R, x_A, \zeta_A, x_B, \zeta_B, \mathbf{r}_v)}
  }{
    \int d\mathbf{r}_v dx_R dx_A d\zeta_A dx_B d\zeta_B I(\zeta_A) I^\ast(\zeta_B)  e^{-\beta U(x_R, x_A, \zeta_A, x_B, \zeta_B, \mathbf{r}_v)}
  }
\end{equation}
...
\begin{equation}\label{eq:atm-partfunc-ratio-rbfe-2}
  \frac{K_b(B)}{K_b(A)} = \frac{
    \int d\mathbf{r}_v dx_R dx_A d\zeta_A dx_B d\zeta_B I(\zeta_A) I^\ast(\zeta_B)  e^{-\beta U(x_R, x_A, \zeta_A + \vec{h}, x_B, \zeta_B - \vec{h}, \mathbf{r}_v)}
  }{
    \int d\mathbf{r}_v dx_R dx_A d\zeta_A dx_B d\zeta_B I(\zeta_A) I^\ast(\zeta_B)  e^{-\beta U(x_R, x_A, \zeta_A, x_B, \zeta_B, \mathbf{r}_v)}
  }
\end{equation}
...
\Delta U = U(x_R, x_A, \zeta_A + \vec{h}, x_B, \zeta_B - \vec{h}, \mathbf{r}_v) - U(x_R, x_A, \zeta_A, x_B, \zeta_B, \mathbf{r}_v)
\end{equation}
...
Many of the issues around simulation setup and protocol choice for
alchemical calculations are common, but there are some differences
between absolute and relative calculations, because of the differences
in the formulations. We will consider protocol differences before
treating the common elements.

\subsubsection{Simulation choices unique to relative free energy calculations}
...
In contrast, in a dual topology alchemical free energy calculation, no atoms are allowed to change type\cite{boresch1999role, shirts2012best}, at least not in the original strict definition. This means that the benzene to benzyl alcohol transformation involves starting with benzene plus the non-interacting dummy atoms making up the hydroxymethyl group, then passing through an intermediate state where some atoms are partially interacting---particularly, those atoms which are becoming dummy atoms or ceasing to be dummy atoms~\cite{mobley2014blind}. The transformation finally culminates in a state where benzyl alcohol is present along with the additional dummy atom which was previously a corresponding hydrogen of the benzene. Fig.~\ref{fig:fig_topology}'s middle branch depicts how such a dual topology works. Note that the two groups that are switched on or off never see each other. The 'mixed' intermediate atoms (M) are used as well, but they strictly interpolate between physical and dummy atoms, or vice versa, never between two different physical atoms.
...
In practice, many contemporary alchemical free energy setup packages use an approach that falls in between the single coordinate single topology (SCST) and single coordinate dual topology (SCDT) approaches. In such a case, perturbation in one part of the molecule may follow SCST mapping, e.g. similar to the left path in Figure~\ref{fig:fig_topology}, while in another part of the molecule atom type changes may be allowed. Since this approach creates a hybrid of both SCST and SCDT, we suggest terming it single coordinate hybrid topology.
...
Some studies use the term ``hybrid'' topology approach~\cite{gapsys2015pmx, gapsys2016accurate, gapsys2020large} for calculations which should be classified as single coordinate hybrid topology.  %\sbnote{Check whether this is correct!}. %this term may not yet have achieved widespread use. In this case, "hybrid" seems to indicate that the set up of these free energy calculations involves a hybrid of the two molecules, and much of what is done in these studies uses a single topology approach~\cite{gapsys2020large}.
Ries et al.\ suggested terminology~\cite{Ries_2022} in which single topology corresponds to SCST, hybrid topology to SCHT/SCDT, and dual topology to DC.
...
\subsubsection{Handling weak binders}\label{sec:weak-binders}
...
\subsubsection{Handling multiple potential binding modes within relative binding free energy calculations.}
...
Overall, this means that relative free energy binding calculations will often be susceptible to problems resulting from uncertainty in ligand binding modes unless more robust approaches are available to determine dominant binding modes, or the relative populations of different potential binding modes, in advance.
...
\subsection{Choosing the alchemical pathway \label{sec:important_path}}
\begin{figure}
...
\subsection{The importance of configurational sampling}
...
A third approach builds on simulated tempering~\cite{marinari1992simulated}, but modified for alchemical changes. In this scheme, a single replica rapidly explores all of $\lambda$ space by switching between different intermediate $\lambda$ values, as seen in Fig.~\ref{fig:fig_sampling_scheme}\textbf{C}. This is also referred to as expanded ensemble, extended ensemble or even just simulated tempering (although the temperature is not changed when applied to $\lambda$ changes in alchemical free energy calculations) ~\cite{lyubartsev1992new, li2007simulated, tan2017optimally} and is supported by OpenMM Tools~\cite{andrearizzi2019choderalab}, GROMACS and AMBER TI~\cite{Lee_JPhysChemB_2025_v129_p13063}.   It has been shown that, under some conditions, the expanded ensemble calculations will move back and forth between replicas faster than replica exchange.~\cite{zhang2008comparison,hsu2024replica}  

While simulated tempering eliminates the need for parallel simulations, it also eliminates the cancellation of unknown free energies that occurs in replica exchange acceptance probabilities. If an expanded ensemble simulation runs without any biasing, it will spend time in each state proportional to $exp(-\Delta G)$ of that state, which means almost all the sampling will be concentrated into the low free energy states.  
Suppose that the true free energies were known and to weight the transitions between $\lambda$ states.  This would result in the simulation visiting all $\lambda$ states evenly, much better than visiting only the low free energy states.  Consequently, any method based on simulated tempering transition kernel requires a reasonable estimate of the free energies to effectively sample all of the $\lambda$reasonable $\lambda$ transitions to force non-zero sampling of all states and extract the information needed for free energies.

However, the choice of the desired asymptotic probability of visiting each $\lambda$ state is important, and uniformly visiting all $\lambda$s is not necessarily the most efficient. This choice is closely related to the choice of spacing of the $\lambda$ states discussed near the end of Section \ref{sec:important_path}: many closely spaced states, each visited with small probability, should behave roughly like fewer, more widely spaced states that are each visited with higher probability. As discussed there, the spacing should be chosen so that the statistical difficulty is approximately uniform along the path. Analogously, the probability of visiting a given $\lambda$ should be roughly proportional to the thermodynamic length volume element\cite{gelman1998simulating, martinez-veracoechea2008variance}.

Any set of reasonable estimates of the free energies, if not off by too much, will serve as reasonable weights for the expanded ensemble simulations, and thus to give reasonable free energy estimates, A number of approaches have been proposed to estimate the free energies on-the-fly, including Wang-Landau sampling~\cite{wang2001efficient}, the accelerated weight histogram method~\cite{lindahl2014accelerated} (AWH), self-adjusted mixture sampling~\cite{tan2017optimally} (SAMS), Times Square sampling~\cite{predescu2024times}, and even using metadynamics in $\lambda$ space~\cite{hsu2023alchemical}. A full description of the different methods to adjust the free energies and target probability distribution is beyond the scope of this review---the interested reader is directed to a more complete review in~\cite{henin2022enhanced} of the different choices as well as the other $\lambda$-based enhanced sampling methods.  

The non-equilibrium (NEQ) switching (Fig.~\ref{fig:fig_sampling_scheme}\textbf{D}) approach makes use of short out-of-equilibrium simulations~\cite{aldeghi2018accurate}. In this protocol, only end state $\lambda$ replicas ($\lambda$=0, $\lambda=1$) are simulated at equilibrium; intermediate information is generated from non-equilibrium simulations that rapidly transition between end states. 
...
\begin{figure}
    ...
    \caption{\textbf{Four most common sampling strategies.} (\textbf{A}): Multiple replicas in parallel at different lambda states. Each arrow symbolises an independent $\lambda$ simulation. (\textbf{B}): Hamiltonian replica exchange scheme. Each arrow represents a short simulation interval before an exchange through Metropolis Hastings acceptance (dice) is attempted. A tick means an accepted exchange, a cross a rejected exchange. (\textbf{C}): Single replica scheme sampling from all $\lambda$ states. After a short simulation time symbolised by the arrow, the lambda-state is attempted to change until all N lambda states will be sampled. (\textbf{D}): Non-equilibrium sampling scheme, where two equilibrium simulations at the end states are run as indicated by the blue and pink arrow. Non-equilibrium simulations are attempted at intervals to switch between the two end states.}
    \label{fig:fig_sampling_scheme}
\end{figure} 

%Currently, we recommend using Hamiltonian replica exchange type sampling schemes (Fig.~\ref{fig:fig_sampling_scheme} \textbf{B}). If these are not available in the code of choice, running independent simulations at different $\lambda$ values can be acceptable, especially when configurational sampling is fast (Fig.~\ref{fig:fig_sampling_scheme} \textbf{A}). Single replica schemes and non-equilibrium schemes are not as established yet because of potential failure modes, but are very promising for use in the near future. 
...
\begin{itemize}
    \item Thermodynamic integration (TI) requires $\frac{\partial U(\mathbf{q})}{\partial\lambda}$.
    \item Exponential averaging (EXP) needs \textit{either} $\Delta U_{k,k+1}(\mathbf{q})$ or $\Delta U_{k,k-1}(\mathbf{q})$, depending on the direction its being evaluated in.
    \item Bennett acceptance ratio (BAR) needs \textit{both} $\Delta U_{k,k+1}(\mathbf{q})$ and $\Delta U_{k,k-1}(\mathbf{q})$.
...
The reduced free energy difference $\Delta f_{01}$ between an initial state 0 and a final state 1 defined by two reduced Hamiltonians $\mathcal{H}_0(\mathbf{q},\mathbf{p})$ and $\mathcal{H}_1(\mathbf{q},\mathbf{p})$, where $\mathcal{H} \equiv \beta H$, can be calculated as:
\begin{eqnarray}
\Delta f_{01}  =  -\ln \expect{e^{- (\mathcal{H}_1(\mathbf{q},\mathbf{p}) - \mathcal{H}_0(\mathbf{q},\mathbf{p}))}}_0 =  -\ln \expect{e^{- \Delta \mathcal{H}(\mathbf{q},\mathbf{p})} }_0
\end{eqnarray}
...
The work of an alchemical transition can be calculated by integrating $\partial \mathcal{H}(\mathbf{q},\mathbf{p})/\partial\lambda$ over the transition path:

\begin{equation}
w   = \int_{0}^{1} \frac{\partial \mathcal{H}(\mathbf{q},\mathbf{p})}{\partial\lambda} d\lambda.  
\end{equation}\label{eqn.work_value}
...
When work distributions are Gaussian, $\pi(w) \sim \mathcal{N}(\overline{w},\sigma_w^2)$, the Jarzynski estimator becomes:

\begin{eqnarray}
\Delta f = \overline{w} - \frac{\sigma_w^2}{2}
\end{eqnarray}\label{eqn.jarzynski_gaussian}

The assumption of Gaussian work distributions usually holds only for slow switching that keeps the system close to equilibrium~\cite{jarzynski1997nonequilibrium}, so it's use should be carefully monitored.
...
\begin{eqnarray}
 \sum_{k=1}^{n_i} \frac{1}{1 + \exp[\ln(\frac{n_i}{n_j}) + \Delta u_{ij}(\mathbf{q}_k) - \Delta f_{ij})
 ]} \nonumber \\
 =\sum_{l=1}^{n_j} \frac{1}{1 + \exp[\ln(\frac{n_j}{n_i}) - \Delta u_{ij}(\mathbf{q}_l) + \Delta f_{ij})]},
...
Replacing $\Delta u_{ij}(\mathbf{q})$ in equation~\ref{eq:bar} by reduced work values $w_k$ and $w_l$ corresponding to the work observed for the forward and reverse transitions, BAR can be written as follows~\cite{shirts2003equilibrium}:

\begin{eqnarray}
 \sum_{k=1}^{n_F} \frac{1}{1 + \exp[\ln(\frac{n_F}{n_R}) + w_{k} - \Delta f)]} \nonumber \\
 =\sum_{l=1}^{n_R} \frac{1}{1 + \exp[\ln(\frac{n_R}{n_F}) - w_{l} + \Delta f]},
...
\begin{equation}
\frac{df}{d\lambda} = \frac{d}{d\lambda} \left[-\ln \int \frac{\exp^{-u(\lambda,\mathbf{q})}}{Z(\lambda)} d\mathbf{q}\right] = \expect{\frac{du(\lambda,\mathbf{q})}{d\lambda}\
}_{\lambda} .
\end{equation}
...
\begin{equation}
\Delta f    = \int_{0}^{1} \expect{\frac{du(\lambda,\mathbf{q})}{d\lambda}}_{\lambda}  d\lambda.    
\end{equation}
...
than averaging $e^{-u(\mathbf{q})}$, as the distribution of values is usually narrower, with a more Gaussian shape to the distribution. Rather than being limited by overlap, as in the case of BAR and MBAR (see below), we are instead limited by the bias in the numerical quadrature, which must be minimized sufficiently to be beneath the level of statistical noise.
...
\expect{\frac{du(\lambda,\mathbf{q})}{d\lambda}}_{k}, \] where the weights $c_k$ correspond to a particular choice of numerical integration.
...
The free energy estimate can be viewed as an empirical average, like the one that arises in thermodynamic integration: if $\{Y_0, \dots, Y_{N-1}\}$ are samples drawn from an ergodic Markov process with stationary density $\pi(\mathbf{q})$ then for $N$ sufficiently large we expect
\begin{equation}
    \int_\Gamma a(\mathbf{q}) \pi(\mathbf{q}) \mathrm{d}\mathbf{q}\approx \frac{1}{N}\sum_{i=0}^{N-1} a(Y_i), \label{eqn:empirical-average}
\end{equation}
...
\end{equation}
where $\rho_0(\mathbf{q})$ is the initial density for $Y_0$, that is, the probability distribution of starting configurations.
...
%
% 1. clearly expressing the dependence of the density as a function (t, \mathbf{q}_0), and what assumptions are made about its behavior; without this, I see no way of making statements about uncertainty quantification, as these all require some statistical theorem, like a Law of Large Numbers, or a Central Limit Theorem. 
...
Consider a molecular dynamics trajectory started at some initial configuration $\mathbf{z}_0 = (\mathbf{q}_0, \mathbf{v}_0)$, where $\mathbf{q}_0 \in \mathbb{R}^{3 N_{\mathrm{atoms}}}$ denotes the initial positions of the system's $N_{\mathrm{atoms}}$ atoms and $\mathbf{v}_0 \in \mathbb{R}^{3 N_{\mathrm{atoms}}}$ its initial velocities, and denote the density at time $t$ by $\rho_t(\mathbf{z}|\mathbf{z}_0)$. This transition density is determined by the molecular dynamics integrator. The distribution of the velocities is usually ignored, since in the presence of a thermostat they are kept distributed roughly according to a Maxwell-Boltzmann distribution, so we only consider the $\mathbf{q}$ component. In principle, one could consider the initial condition $\mathbf{q}_0$ to be drawn from a more complicated initial distribution $\rho_0(\mathbf{q}) \mathrm{d}\mathbf{q}$, like a mixture of Dirac densities $\rho_0(\mathbf{q}) \mathrm{d}\mathbf{q} = \sum_{i = 1}^n \delta_{\{ \mathbf{q}_i \}}(\mathrm{d}\mathbf{q}) / n$, but any such "better" initial density would require some knowledge about the system at hand, such as the distribution of binding modes. 

It is difficult to make general statements about the behavior of $\rho_t (\mathbf{q} | \mathbf{q}_0)$. For systems not involving a protein or otherwise having a smaller number of degrees of freedom, it may be possible to identify clear "plateaus'' of convergence, i.e. long periods within which $\rho_t (\mathbf{q}|\mathbf{q}_0)$ does not vary much with the time $t$, at least in those reaction coordinates relevant to the free energy being estimated, and within which $\rho_t (\mathbf{q}|\mathbf{q}_0)$ is approximately invariant to $\mathbf{q}_0$ in some appropriate starting domain. Consider, for instance, the estimation of free energies of hydration for small molecules ~\cite{paliwal2011benchmark}. In this case, the free energy estimates behave in accordance with classical statistical theory, and for sufficiently large $t$ and most $\mathbf{q}_0$ the density $\rho_t (\mathbf{q} | \mathbf{q}_0)$ should not vary much along coordinates relevant to the free energy estimates. Quantifying ideas such as "long periods" or "vary much"  and of identifying relevant reaction coordinates is an active area of research and beyond the scope of this review.
...
% Of course, it _could_ be the case for some systems that \rho_t(\mathbf{q} | \mathbf{q}_0) is independent of \mathbf{q}_0, and that the empirical distribution of any replicate is a good approximation for \rho_t (\mathbf{q}|\mathbf{q}_0). In general, however, we will only have access to a so-called quasi-stationary distribution; i.e. we will consider the distribution conditional on not having exceeded an escape time, and this distribution will only equilibrate in some degrees of freedom. 

% The following footnote feels out of place, but might address some comments by A. Bhati re: the Poincaré recurrence times.
% \footnote{(Luckily, it is not necessary for simulations to visit every configuration $\mathbf{q} \in \Gamma$ in order to exhibit a regime of convergence for the free energy estimates. For instance, two configurations $\mathbf{q}$ and $\mathbf{q}'$ may be considered equivalent for free energy estimation if they differ only by the permutation of water molecules. Nature too faces the same difficulty that not all states are accessible on timescales we can observe -- it will take an astronomically long time before all the gas particles in a room happen to be on one half-side of it -- but this does not prevent us from measuring the temperature of (most) rooms.)}
...
\textbf{Definition}:
We define an \textbf{ensemble} to be a set of $N_{\mathrm{rep}} \geqslant 1$ trajectories, all started at the same initial configuration $\mathbf{q}_0$, with initial velocity $\mathbf{v}_0$ independently drawn from the Maxwell-Boltzmann distribution at the appropriate temperature and run for a chemical time $\mathcal{T}$. We emphasize that, with this definition, all ensemble-based estimates are inherently tied to the choice of $\mathbf{q}_0$; pathological choices  -- for example, a protein–ligand configuration that may quickly unbind -- can render the resulting estimates meaningless.

\subsubsection{Ensemble error estimates}

This section describes the simplest and, in many cases, the most robust procedure for estimating uncertainties in free energy calculations. For most practical applications, we recommend this approach when reporting statistical error bars. The procedure consists of simulating an ensemble $\{X^{(1)},\ldots,X^{(N_{\mathrm{rep}})}\}$ of $N_{\mathrm{rep}}$ molecular dynamics trajectories initialized at $\mathbf{q}_0$, and using the resulting free energy estimates $\{\Delta f^{(1)},\ldots,\Delta f^{(N_{\mathrm{rep}})}\}$ to quantify uncertainty. The trajectories $\Delta
G^{(i)} (\cdot)$ are identically distributed and conditionally independent given $\mathbf{q}_0$. For any time $\mathcal{T} > 0$, the variance of any $\Delta f^{(i)} (\mathcal{T})$ can be estimated with the sample variance $S^2 (\mathcal{T})$,
...
\end{equation}
In the notation of Eq.~\ref{eqn:empirical-average}, $Y_i = \Delta f^{(i)}(\mathcal{T})$; conditioned on $\mathbf{q}_0$, the $Y_i$ are independent and identically distributed and are trivially a Markov process, with stationary distribution given by the distribution of $\Delta f^{(1)}(\mathcal{T})$. By direct calculation, we have (conditionally on $\mathbf{q}_0$)
\begin{equation}
    \mathbb{E}_{\mathbf{q}_0}[S^2(\mathcal{T})] = \mathrm{Var}_{\mathbf{q}_0}(\Delta f^{(1)}(\mathcal{T}))
\end{equation}
The above equation says that $S^2(\mathcal{T})$ is an unbiased estimator of the variance of $\Delta f^{(1)}(\mathcal{T})$, conditioned on $\mathbf{q}_0$. By the Law of Large Numbers, as $N_{\mathrm{rep}} \rightarrow \infty$, we have $S^2 (\mathcal{T}) \rightarrow \mathrm{Var} _{\mathbf{q}_0}(\Delta
G^{(1)} (\mathcal{T})) = \sigma^2 (\mathcal{T} ; \mathbf{q}_0)$. Furthermore, by conditional independence of the $\Delta f^{(i)}(\mathcal{T})$, for sufficiently large $N_{\mathrm{rep}}$ and any $\mathcal{T}>0$ for which the variance is finite, we have
\begin{equation}\label{eqn:variance-of-empirical-mean}
\mathrm{Var}_{\mathbf{q}_0}(\overline{\Delta f(\mathcal{T})}) = \frac{\mathrm{Var}_{\mathbf{q}_0}(\Delta f^{(1)}(\mathcal{T}))}{N_{\mathrm{rep}}} \approx \frac{S^2(\mathcal{T})}{N_{\mathrm{rep}}}
\end{equation}
...
\begin{equation}
  \mathrm{SE} (\mathcal{T} ; \mathbf{q}_0) = \sqrt{\frac{S^2(\mathcal{T})}{N_{\mathrm{rep}}}},
\end{equation}
which is the gold standard for reporting the error of a free energy estimate $\overline{\Delta f (\mathcal{T})} $ when a single initial condition $\mathbf{q}_0$ is used. Note that $\mathrm{SE}
(\mathcal{T} ; \mathbf{q}_0) \rightarrow 0$ as $N_{\mathrm{rep}} \rightarrow \infty$ so long as $\mathrm{Var}_{\mathbf{q}_0}(\Delta f^{(1)}(\mathcal{T}))$ is finite, which is true if the $\Delta f^{(1)}(\mathcal{T})$ are bounded. Thus the variance can be made arbitrarily small simply by running enough replicates, but we note critically this does \textit{not} mean that we will have estimated the free energy to arbitrary precision: $\Delta f^{(1)}(\mathcal{T})$ in general will not be normally distributed, its value may vary greatly with both $\mathcal{T}$ and $\mathbf{q}_0$, and the variance may not be a good statistic to capture the error, as the distribution may be multi-modal, heavy-tailed and be sensitive to rare events. For example, one replicate out of many might explore a different mode of an important torsional angle, leading to a drastically different free energy estimate. 
...
In principle, the most robust error estimates can be obtained by starting from many different initial conditions $\mathbf{q}_0, \mathbf{q}_1, \dots$, and computing means and standard errors, but as mentioned in Section \ref{sec:timescales}, the value of this approach is limited by the user's ability to choose a suitable initial density $\rho_0(\mathbf{q})$.

\subsubsection{Per-replicate error estimates}

In the preceding case, we used minimal assumptions on the distribution of $\Delta f^{(i)}(\mathcal{T})$ to obtain estimates on $\sigma^2(\mathcal{T};\mathbf{q}_0)$ by simulating conditionally independent replicates and using the sample variance Eq.~\ref{eq:sample-variance-definition}. To obtain per-replicate (i.e., per-$i$) error estimates, we impose much stricter assumptions on the distribution of $\Delta f^{(i)}(\mathcal{T})$,  namely that they satisfy a Central Limit Theorem (CLT) as $\mathcal{T}\rightarrow \infty$, for each $i=1,\dots,N_{\mathrm{rep}}$:
$$ \sqrt{\mathcal{T}} ( \Delta f^{(i)}(\mathcal{T}) - \Delta f^\star) \Rightarrow N(0, \sigma^2_{\mathrm{asym}}(\mathbf{q}_0))$$
where $\Delta f^\star$ denotes the true free energy difference, $\sigma_{\mathrm{asym}}^2(\mathbf{q}_0)>0$ denotes the so-called "asymptotic variance", $ N(0, \sigma^2_{\mathrm{asym}}(\mathbf{q}_0))$ is a Gaussian distribution with mean 0 and variance $\sigma_{\mathrm{asym}}^2(\mathbf{q}_0)>0$, and $\Rightarrow$ denotes convergence in distribution. %% Should we define this? Might be a little too technical for the audience. Again, Durrett is one of the standard references. 
The CLT assumption implies that 
$$\sigma^2(\mathcal{T};\mathbf{q}_0)\approx\frac{\sigma^2_{\mathrm{asym}}(\mathbf{q}_0)}{\mathcal{T}}.$$
In practice, it may not be the case that $\sigma^2(\mathcal{T};\mathbf{q}_0)\propto \mathcal{T}^{-1}$, nor even that $\sigma^2(\mathcal{T};\mathbf{q}_0)$ is monotonically decreasing with $\mathcal{T}$. In such a case, we are not in a regime where the CLT is satisfied and the remainder of this subsection does not apply.

To understand the difficulty of estimating $\mathrm{Var}_{\mathbf{q}_0}(\Delta f^{(i)}(\mathcal{T}))$ using only samples from replicate $i$, it is helpful to recall the empirical mean Eq.~\ref{eqn:empirical-average} as the canonical estimator. Suppose that $\{Y_i\}_{i\ge 0}$ is an ergodic Markov chain with stationary distribution $\pi$, that $Y_0 \sim \pi$, and that $t_{N-1}=\mathcal{T}$. Let $a$ be a fixed observable. 
\begin{align}
\sigma^2(\mathcal{T};\mathbf{q}_0) &= \mathrm{Var}_{\pi}\!\left(\frac{1}{N}\sum_{i=0}^{N-1} a(Y_i) \right) \\
...
Partition the $N$ sequential samples of this single replicate into $N_{\mathrm{b}}$ consecutive batches of equal size $M$, so that $N = BM$. Batch $k$ consists of the samples $\{X(t_{kM}),\dots,X(t_{(k+1)M-1})\}, k=0,\dots,N_{\mathrm{b}}-1$, with $X(t_0) = \mathbf{q}_0$. Let
\[
...
$$ \hat\sigma_{N_{\mathrm{b}}}^2(\mathcal{T}; \mathbf{q}_0) \approx S^2(\mathcal{T} ; \mathbf{q}_0) .$$
...
\begin{eqnarray}
g &\equiv& 1 + 2 \tau_{\mathrm{int}}(N) \approx  \frac{\sigma^2(\mathcal{T};\mathbf{q}_0)}{\sigma_0^2} N \label{eq:statistical-inefficiency-definition} 
\end{eqnarray}
is referred to as the \textit{statistical inefficiency}. Loosely speaking, $g$ measures the number of indices $i$ between roughly independent samples. The role of estimators such as non-overlapping batch means is precisely to estimate $\sigma^2(\mathcal{T};\mathbf{q}_0)$ directly from correlated data, which is roughly equivalent to estimating $g$. 
...
\paragraph{Computing the timeseries for equilibration detection}
Typically, the timeseries of note $a_t$ analyzed in automated equilibration detection is the negative logarithm of the probability density ($\pi(X_t; \lambda$)) sampled by the MCMC algorithm (up to an irrelevant additive constant).
For simple independent simulations that sample $\mathbf{q}_t \sim \pi(\mathbf{q} ; \lambda)$, this is given by the reduced potential
\begin{eqnarray}
a_t &\equiv& - \ln \pi(\mathbf{q}_t; \lambda) + c = u(\mathbf{q}_t; \lambda) .
\end{eqnarray}
...
\[
E(t)^2 \;\equiv\; \mathrm{Var}_{\mathbf{q}_0}\!\bigl(\Delta f^{(i)}(t)\bigr)
\]
...
Therefore, $\mathrm{var}$($\Delta$f) = $\sum_{i=1}^{K}w_{k}^2 \mathrm{var}(\frac{du}{d\lambda})_{k}$, where the $w_k$ are the integration weights (see Sec~\ref{subsec:estimators} for more discussion of $w_k$).
...
% Theory Deep Dive        %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Detailed derivations of statistical mechanical results}
\label{sec:derivations}

In this section, we present the necessary statistical mechanics theory of dilute solutions to define the quantities we are trying to calculate using alchemical methods in more detail than in Sec.~\ref{sec:theory}. There will therefore be some repetition between the two sections.  This section is mainly intended to provide more rigor to the derivations in section Sec.~\ref{sec:theory} and many readers may be able to skip this.  However, these details are often left out of much of the literature, and we gather them together here to provide a reference for those instances when fiddling with the details of implementations or derivations is needed.

Here, we derive free energy expressions that form the basis of alchemical computational protocols for modeling noncovalent molecular binding, solvation, and solvent partitioning equilibria. The emphasis here is placed on bridging theoretical foundations and intuition. Since chemical equilibria are governed by the chemical potentials of the species involved, we begin by considering a statistical mechanics expression for the chemical potential of a solute in a solvent. We will then combine chemical potentials to develop models for each process.  As discussed in Sec.~\ref{sec:theory}, our derivations are entirely classical, and we will repeat some definitions from Sec.~\ref{sec:theory} to avoid having the reader flip back and forth between sections.

\subsection{The chemical potential}
\label{sec:chemical_potential-app}

As described before in~\ref{sec:theory}, the chemical potential of a solute $s$ in an ideally diluted solution in a solvent $v$ can be written as~\cite{guggenheim1952mixtures504,gilson1997statisticalthermodynamic,gilson2007calculation}
\begin{equation}
  \mu_s = -k_B T \ln \frac{q'_s}{C_s \Lambda_s^3}
  \label{eq:chemical-potential-definition-app}
\end{equation}
where $k_B$ is Boltzmann's constant, $T$ is the absolute temperature, $C_s = N_s/V$ is the number concentration of the solute, $\Lambda_s = h/\sqrt{2 \pi M_s k_B T}$ is the thermal de Broglie wavelength of the solute, where $h$ is Planck's constant and $M_s$ is the solute's total mass. 
An ideally dilute solution is a potentially non-physically realizable solution in which solute molecules interact only with the solvent, and solute-solute interactions are negligible~\cite{levine2009physicalchemistrybook6ed}.

The quantity $q'_s$ in Eq.~\ref{eq:chemical-potential-definition-app} is the molecular partition function of the solute at rest in the solvent
\begin{equation}
  q'_s = \frac{\Lambda_s^3}{\prod_i \lambda_i^3} 8 \pi^2 z_s
  \label{eq:intra-qpu-def-app}
\end{equation}
where $\beta = (k_B T)^{-1}$, $\lambda_i$ is the thermal De Broglie wavelength of the $i$-th atom of the solute (not the alchemical parameter as used elsewhere in this document), and $z_s$ is the intramolecular configurational partition function of the solute.  As described in Sec.~\ref{sec:theory}, masses do not enter the classical statistical mechanical potentials, so the thermal De Broglie wavelength terms coming from the partition functions of the momenta cancel in the resulting expressions below, and we will no longer consider them.  Up to this point, we are merely recapitulating what was presented in Sec.~\ref{sec:theory}; indeed, Eqs.~\ref{eq:chemical-potential-definition-app} and \ref{eq:intra-qpu-def-app} are the same as Eqs. ~\ref{eq:chemical-potential-definition} and \ref{eq:intra-qpu-def} and are included here again for completeness (similar repetitions will occur throughout this section).

However, in contrast to Sec.~\ref{sec:theory} We define this intramolecular partition function more precisely here, as:
\begin{equation}
  z_s = \int dx_s e^{-\beta \Psi_v(x_s)}
  \label{eq:intra-zu-def}
\end{equation}
where $x_s$ represents the collection of internal degrees of freedom of the solute (bond lengths, bond angles, and dihedral angles) with the corresponding volume element $dx_s$ assumed to include the appropriate Jacobian factors. The function $\Psi_v(x_s)$ in Eq.~\ref{eq:intra-zu-def} is the solvent-averaged potential of mean force of the solute in conformation $x_s$ in solvent $v$~\cite{rouxandsimonson1999solventmodels} 
\begin{equation}
  e^{-\beta \Psi_v(x_s)} = \frac{1}{Z_{N_v}} \int d\mathbf{r}_v e^{-\beta U(x_s, \mathbf{r}_v)}
  \label{eq:solvent-pmf-def}
\end{equation}
where $\mathbf{r}_v$ represents the collection of Cartesian coordinates of the atoms of $N_v$ solvent molecules, $U(x_s, \mathbf{r}_v)$ is the potential energy of the solvent with one solute molecule in conformation $x_s$ placed in an arbitrary position in a volume $V$. When the potential energy function of the solution is separable into an intramolecular potential energy of the solute $U(x_s)$ and a solute-solvent non-bonded interaction energy, the potential of mean force can be written as the sum $\Psi_v(x_s) = U(x_s) + W_v(x_s)$ where $W_v(x_s)$ is the solvent potential of mean force, the solvation free energy of the solute fixed in configuration $x_s$~\cite{gilson2007calculation}. However, the potential of mean force definition Eq.~\ref{eq:solvent-pmf-def} is valid in general. Finally, $Z_{N_v}$ is the configurational partition function of the pure solvent
\begin{equation}
  Z_{N_v} = \int d\mathbf{r}_v e^{-\beta U(\mathbf{r}_v)} \, .
  \label{eq:Z-pure-solvent}
\end{equation}

Eq.~\ref{eq:chemical-potential-definition-app} is derived by writing the classical canonical configurational partition function $Z(N_s = 1, N_v)$ of one solute molecule in a solvent of $N_v$ molecules in a volume $V$ and multiplying and dividing it by configurational partition function of the pure solvent [Eq.~\ref{eq:Z-pure-solvent}], and then separating out the integration over the external degrees of freedom of the solute (translations and rotations) giving an $8 \pi^2 V$ term to yield $Z(N_s = 1, N_v) = Z_{N_v} 8 \pi^2 V z_s$ where $z_s$ is defined by Eqs.~\ref{eq:intra-zu-def} and (\ref{eq:solvent-pmf-def}). The canonical configurational partition function of $N_s$ independent solute molecules in a solvent is then obtained by considering the product of the solute terms $Z(N_s, N_v) = Z_{N_v} (8 \pi^2 V z_s)^{N_s}/N_s!$,~\cite{guggenheim1952mixtures504,simonson2016physical} where the factorial term accounts for indistinguishability. Finally, the canonical partition function $Q(N_s, N_v)$ of the solution is obtained by including the appropriate terms coming from the momenta and differentiating the resulting Helmholtz free energy $A(N_s, N_v) = -k_B T \ln Q(N_s, Nv)$ with respect to $N_s$ to obtain Eq.~\ref{eq:chemical-potential-definition}.

The form Eq.~\ref{eq:chemical-potential-definition-app} for the solute chemical potential using the potential of mean force description is useful, as we will see, because it separates the solute's translational motion, which leads to the concentration dependence, from the intramolecular contributions. The potential of mean force description does not introduce approximations. With the exception of the use of classical statistical mechanics and the assumption  that solute-solute interactions are negligible, Eq.~\ref{eq:chemical-potential-definition-app} is rigorous. Due to rotation-vibration couplings, the separation of the solute's overall rotational motion leading to the $8 \pi^2$ term in Eq.~\ref{eq:intra-qpu-def} is not exact. While it is not a strict theoretical requirement, the separation of rotational degrees of freedom is adopted here because it is considered an excellent approximation at room temperature and a useful simplification.

We derived Eq.~\ref{eq:chemical-potential-definition-app} using the canonical (constant volume) partition function of the solution. Since the chemical potential can be equivalently obtained from the constant-volume differentiation of the Helmholtz free energy or the constant-pressure differentiation of the Gibbs free energy, Eq.~\ref{eq:chemical-potential-definition-app} is correct. However, the volume dependence of the chemical potential has to be considered when modeling the Gibbs free energy change for constant-pressure processes where the system's volume changes significantly~\cite{gilson1997statisticalthermodynamic}. Terms arising from volume variations at constant pressure are generally considered small for solvation, solvent partitioning, and molecular binding processes at atmospheric pressure and are often ignored in alchemical calculations.  This is because when a single molecule is removed, errors scale with the fractional change in the system size.  When one particle in 1000 disappears, this will be negligible with respect to other sources of error. Hence, we will use Eq.~\ref{eq:chemical-potential-definition} to derive expressions for $\Delta G$'s.

The standard chemical potential of a solute in an ideally dilute solution is obtained from Eq.~\ref{eq:chemical-potential-definition-app} by setting the concentration to the standard concentration $C^\circ$, generally $1$ mol/L equivalent to $1/1661$ particles/\AA$^{3}$, 
\begin{equation}
  \mu_s^\circ = -k_B T \ln \frac{q'_s}{C^\circ \Lambda_s^3} \, .
  \label{eq:chemical-potential-standard-app}
\end{equation}
The standard state of a solution is defined as an ideally dilute solution at concentration $C^\circ$.~\cite{levine2009physicalchemistrybook6ed} Hence, Eq.~\ref{eq:chemical-potential-standard-app} is formally correct for real solutions.

Next, we outline the application of the chemical potential statistical mechanics formalism to model key biophysical processes: solvation, solvent partitioning, and noncovalent molecular binding. In general, translating statistical-mechanics identities into computable quantities via molecular simulations involves expressing ratios of partition functions in terms of ensemble averages. In the derivations below, we make the choice of using the Zwanzig relationship~\ref{eq:zwanzig}, resulting in the average of the exponential of a perturbation energy function.  However, many other free energy estimators can be used as discussed in Sections \ref{subsec:formulations-abfe-rbfe}, \ref{subsec:numerical-implementations},  and \ref{subsec:estimators}. As in the previous subsection, we use the solvent potential of mean force to simplify the notation when possible. The solvent degrees of freedom can be restored in the resulting expressions using Eq.~\ref{eq:solvent-pmf-def}. 

\subsection{Solvation free energy}\label{sec:derivation-solvation}
...
\begin{equation}
  \Delta G^\circ_{\mathrm{slv}} = \mu^\circ_{A(v)}  - \mu^\circ_{A(g)} = -k_B T \ln \frac{P^\circ}{C^\circ k_B T} - k_B T \ln \frac{z_{A(v)}}{z_{A(\mathrm{g})}}
  \label{eq:DG0-solvation-def-app}
\end{equation}
where we use Eq.~\ref{eq:chemical-potential-standard-app} for the solution, Eq.~\ref{eq:chemical-potential-definition-app} for the gas (assumed ideal) at the standard gas concentration $C = P^\circ/k_B T$ where $P^\circ = 1$ bar is the standard pressure, and Eq.~\ref{eq:intra-qpu-def} noting that all momentum terms cancel. The first term in Eq.~\ref{eq:DG0-solvation-def-app} is an ideal term $\Delta G^\circ_{\mathrm{slv,ideal}}$ that accounts for the difference of standard states for solution and gases. The second term is the excess solvation free energy $\Delta G_{\mathrm{slv, exc.}}$ that can be evaluated by numerical alchemical calculations as described in this document. Hence Eq.~\ref{eq:chemical-potential-standard-app} can be used to compare calculated and experimental standard free energies of solvation~\cite{thompson2004new}.
...
\begin{equation}
  \frac{z_{A(v)}}{z_{A(\mathrm{g})}} =
  \frac{
    \int dx_A d\mathbf{r}_v e^{-\beta U(x_A, \mathbf{r}_v)}
  }{
    \int dx_A d\mathbf{r}_v e^{-\beta [U(\mathbf{r}_v) + U(x_A)] }
  }
\end{equation}
...
Next, we multiply and divide by the integral $\int d\zeta$ of the six external degrees of freedom of the solute (the center of mass position and the orientation angles). These degrees of freedom, together with the internal degrees of freedom $x_A$, allow the recast the integrals in terms of the Cartesian coordinates $\mathbf{r}_A$ of the $n_A$ solute's atoms which are typically used in computer simulations, without restraining the position and orientation of the solute. Finally, we multiply and divide the integrand in the numerator by the integrand in the denominator to obtain
\begin{equation}
  \frac{z_{A(v)}}{z_{A(\mathrm{g})}} =
  \frac{
    \int d\mathbf{r}_A  d\mathbf{r}_v  e^{-\beta \{ U(\mathbf{r}_A, \mathbf{r}_v) -  [ U(\mathbf{r}_v) + U(\mathbf{r}_A) ] \}} e^{-\beta [U(\mathbf{r}_v) + U(  \mathbf{r}_A  )] } 
  }{
    \int d\mathbf{r}_A d\mathbf{r}_v e^{-\beta [U(\mathbf{r}_v) + U( \mathbf{r}_A  )] } 
  } = \langle e^{-\beta \Delta U} \rangle_0
  \label{eq:zratio-solv-average-app}
\end{equation}
where $\langle \ldots \rangle_0$ is an average in the decoupled ensemble and $\Delta U =  U(\mathbf{r}_A , \mathbf{r}_v) -  [ U(\mathbf{r}_v) + U(\mathbf{r}_A ) ]$ the change in the system's potential energy for coupling the solute to the solvent. Equivalently, we can consider the reciprocal of Eq.~\ref{eq:zratio-solv-average-app} and express it in terms of an ensemble average $\langle \ldots \rangle_1$ in the coupled ensemble:
\begin{equation}
  \frac{z_{A(v)}}{z_{A(\mathrm{g})}} = \frac{1}{\langle e^{\beta \Delta U} \rangle_1}
\end{equation}
where $-\Delta U = [ U(\mathbf{r}_v) + U(\mathbf{r}_A ) ] - U(\mathbf{r}_A , \mathbf{r}_v)$ is a decoupling energy.
...
\end{equation}
As discussed earlier, alchemical solvation free energy calculations implement Eq.~\ref{eq:zratio-solv-dgexc}, by means of \emph{coupling} or \emph{decoupling} computational protocols and alchemical intermediate states as discussed in section \ref{subsec:numerical-implementations}.

\subsection{Free energy of solvent partitioning}\label{sec:derivations-partitioning}
...
\begin{equation}
  \Delta G^\circ_{\mathrm{part}} = -k_B T \ln P_{12} = \mu^\circ_{A(v_2)}  - \mu^\circ_{A(v_1)} = -k_B T \ln \frac{z_{A(v_2)}}{z_{A(v_1)}}
  \label{eq:DG0-partitioning-def-app}
\end{equation}
where $P_{12} = C_{A(v_2)}/C_{A(v_1)}$ is the partition coefficient of $A$ for the two solvents~\cite{rustenburg2016measuring}. Eq.~\ref{eq:DG0-partitioning-def-app} 
immediately identifies the partition coefficient with the ratio of intramolecular configurational partition functions of solute $A$ in the two solvents:
\begin{equation}
  P_{12} = \frac{z_{A(v_2)}}{z_{A(v_1)}} \, .
  \label{eq:partition-coefficient-app}
\end{equation}
This relation connects calculated ratios of partition functions to experimental partition coefficients. Similarly to Eq.~\ref{eq:DG-solvation-excess}, Eq.~\ref{eq:partition-coefficient-app} corresponds to the process of transferring the solute from a fixed position and orientation in one solvent to a fixed position and orientation in the other.

\subsection{Non-covalent molecular binding}
\label{sec:non-covalent-molecular-binding-app}

The standard Gibbs free energy of binding between a receptor $R$ and a ligand $L$ to form a complex $RL$ in a solvent $v$
\begin{equation}
  R(v) + L(v)  \leftrightharpoons RL(v)
  \label{eq:binding-reaction-app}
\end{equation}
is
\begin{equation}
  \Delta G^\circ_{b} = \mu^\circ_{RL(v)}  - \mu^\circ_{R(v)} - \mu^\circ_{L(v)} = -k_B T \ln K_b 
  \label{eq:DG0-binding-def-app}
\end{equation}
where~\cite{gilson1997statisticalthermodynamic,gilson2007calculation}
\begin{equation}
  K_b = \frac{C^\circ}{8 \pi^2} \frac{z_{RL(v)}}{z_{R(v)} z_{L(v)}}
  \label{eq:DG0-binding-constant-def-app}
\end{equation}
is the statistical mechanics expression for the binding constant. Eq.~\ref{eq:DG0-binding-constant-def} follows from the application of  Eq.~\ref{eq:chemical-potential-standard-app} for each of the three species and noting that the contributions of the momenta cancel. As discussed below, various alchemical protocols exist to evaluate the ratios of partition functions in Eq.~\ref{eq:DG0-binding-constant-def}, thereby connecting the theory to observed binding affinities.
...
\end{equation}
$I(\zeta)$ may sometimes even still be a simplification if internal degrees of the ligand or protein (as may be the case with flexible ligands or binding sites), but we will omit the extension of the theory for now.

As described in Sec.~\ref{sec:theory}, this definition of the bound macrostate of the complex is an essential input of the theory.  Eq.~\ref{eq:DG0-binding-constant-def-app} is the basis of a variety of alchemical computational implementations that have been developed to study molecular binding by alchemical means. Some of these are discussed in Section \ref{subsec:formulations-abfe-rbfe}. Generally, to formulate the statistical mechanics expression of the binding constant Eq.~\ref{eq:DG0-binding-constant-def-app} as an ensemble average, we first need to ensure that the configurational integrals in the numerator and denominator are over the same set of degrees of freedom. Note that the intramolecular configurational partition function of the complex, $z_{RL(v)}$, includes six more degrees of freedom---the position and orientation of the ligand relative to the receptor collectively described by the variable $\zeta$---than the product of the intramolecular partition functions, $z_{R(v)} z_{L(v)}$, of the separated receptor and ligand. To correct this discrepancy, we multiply and divide the right-hand side of Eq.~\ref{eq:DG0-binding-constant-def}  by the quantity
\begin{equation} \label{eq:Vsite-def}
    V_{\rm site} \Omega = \int d\zeta I(\zeta) \, ,
\end{equation}
which measures the extent of the binding pose macrostate, where $V_{\rm site}$ corresponds to the spatial extent and $\Omega$ the orientational extent. The right-hand side of Eq.~\ref{eq:Vsite-def} combined with the integrals $z_{R(v)}$ and $z_{L(v)}$ yields a partition function similar to Eq.~\ref{eq:intra-zRL-def} but corresponding to a complex in which ligand and receptor are uncoupled~\cite{gallicchio2011recent} as if they were at infinite distance apart. The result is
...
Relative free energy calculations carry with them additional unsettled practices relating to the use and careful treatment of dummy atoms (see Sec.~\ref{subsec: Dummy Atoms} on ``Dummy Atoms''). In particular, as the work of~\cite{Fleck_2021} highlighted, valence terms for dummy atoms must be treated far more carefully than previously appreciated in order for dummy atoms' contributions to the free energy to rigorously cancel. Less careful treatments may leave dummy atom terms only approximately canceling, or, worse, introducing uncharacterized errors in relative binding free energies. Indeed, such issues might have impeded previous attempts at reproducing free energy calculations across simulation engines~\cite{loeffler2018reproducibility}. 
...
Absolute binding free energies often suffer from systematic errors due to insufficient end-state sampling. Starting from a holo structure artificially preorganizes the binding site, overestimating affinity if the system fails to relax to the apo state. Because this error is often roughly constant across a ligand series, a pragmatic correction is a reference-compound offset~\cite{SchrodingerVogele,Chen2023HitDiscovery,wu2025optimizing}. However, this constant-reorganization assumption breaks down when binders induce substantially different conformational changes, and correcting for such unequal preorganization energies remains an active area of research, with tentative remedies proposed for several identified sources. One such source is inadequate water resolvation of the binding pocket, that can be addressed by a short per-window GCMC-like equilibration with water swapping to restore known apo solvation levels~\cite{NithishABFE,BenShalom2020InterfacialWater}. The GCMC-like simulations must be performed with the ligand partially decoupled to preserve configurational overlap across lambda~\cite{ChenggongHuiGCMC} windows. Another source, specific to membrane proteins with lipid-exposed binding pockets, is slow lipid exchange~\cite{NithishABFE,CharlieCookABFE,lopezmartin2025lipidcode}, which can be addressed by including the free energy of lipid binding as an explicit step in the alchemical cycle. The most common, and most challenging, source is the reorganization energy from protein conformational rearrangement upon ligand removal. One possible remedy seeds higher-lambda windows with apo-like structures~\cite{SchrodingerApoWindow,CharlieCookABFE}, though this can produce erroneous affinities from insufficient overlap between apo-like and holo-like windows~\cite{NithishABFE}.An alternative approach for some well-characterized systems is to apply enhanced sampling methods to estimate the reorganization free energy ($\Delta G_{holo \rightarrow apo}$) as an explicit contribution to the alchemical cycle~\cite{NithishABFE,SchrodingerVogele}. Another rigorous approach to tackle such conformational changes is the Non-Equilibrium ABFE protocol proposed by Khalak \textit{et al.}~\cite{khalak2021alchemical}, in which a realistic apo ensemble is seeded directly from experimental coordinates, reducing overestimation of binding free energies. However, large conformational changes can bias the estimator by causing poor overlap between work distributions.
...
\textbf{IA} Reviewed and contributed to Sec.~\ref{sec:prerequisites},~\ref{subsec:hydration}, ~\ref{subsec:binding}, ~\ref{sec:multiple-chemical-states}, and ~\ref{sec:unsettled}. Updated Sec.~\ref{sec:software}.\\
...
\textbf{EG} Wrote the updated Sec.~\ref{sec:derivations} and Sec.~\ref{sec:multiple-chemical-states},  updated Sec.~\ref{sec:standardstate-restraints}, and contributed to the discussion and review of many sections.\\
...
\textbf{NMA}: Contributed to the discussion on apo state issues in Sec.~\ref{subsec:binding} and Sec.~\ref{sec:unsettled}.\\
...
\textbf{MRS}: Helped create figure~\ref{fig:fig_what_is_lambda}, wrote Sec.~\ref{sec:important_path} describing choices for alchemical pathways and parts of~\ref{sec:data_analysis} on the analysis for free energy calculations. Helped reorganize theory and uncertainty sections. Reviewed, organized and edited text throughout.\\
...
DLM acknowledges support from the National Institutes of Health (R01GM108889, R01GM124270, and R01GM132386), and the National Science Foundation (CHE 1352608). NMA is funded by the MRC via an iCASE studentship with Vertex Pharmaceuticals with contributions from Reuben College, Oxford. 
JDC acknowledges support from the National Institutes of Health (P30 CA008748, R01 GM121505, R01 GM132386)A complete funding history for the Chodera lab can be found at \url{http://choderalab.org/funding}.
EG (R15GM151708), and MRS (R35GM158359) also acknowledge funding support from the National Institutes of Health.
