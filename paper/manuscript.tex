...
\item Free energy calculations that combine both thermodynamic and kinetic data~\cite{wu2016multiensemble, mey2014xtram, wu2014statistically,dtram???}
...
To this end, Gilson et al.~\cite{gilson1997statisticalthermodynamic}. introduced an indicator function $I(\zeta)$ equal to one when the six degrees of freedom of the complex that define the position (three degrees of translation) and orientation (three degrees of rotation) of the ligand relative to the receptor, the six if which are collectively denoted as $\zeta$, are within the specified range of the bound state and equal to zero otherwise.

The definition of the bound macrostate of the complex is \textit{essential} to defining ligand binding.  Unlike bond lengths, bond angles, and dihedral angles, the three degrees of freedom of the complex that correspond to the position of the ligand relative to the receptor are unbounded. Unless we define specific limits to these variables, the bound state of the complex cannot be distinguished from the unbound state, and the free energy difference between the two states will be undefined.  In fact, this approximation may be slightly too simple, as there may be cases where only some configurations of a highly flexible ligand actually have experimental affect, so the indicator function $I$ may need to include configurations of the ligand or even protein as well. 

The definition of macrostates that are defined by specific choices of the microstates is required in many areas of computational chemistry and biophysics, for example, when specifying the range of backbone angles to measure the relative populations of $\alpha$-helical and $\beta$-strand conformations of peptides, or even whether a protein is folded or not.  What these definitions have in common is that they are different functionally. Only certain configurations have biological or experimental results, and thus when we analyze any sort of simulation, we should try to figure out what simulation variables correspond to the experimentally identifiable configurations. 

The value of the binding free energy depends on the chosen structural definition of the bound complex.
The interpretation of experimental reporters of binding is a complex issue in itself.  For example, a experimental method that identifies any energetic interaction between a protein and a ligand (such as isothermal titration calorimetry) will recognize a different bound state than one that only detects a ligand that is position in a specific location on the protein (such as changes in polarization upon displacement of a probe ligand). The relationship between the predictions of this theory and experimental reporters (thermochemical, spectroscopic, inhibition kinetics, and others) of complex formation has been discussed.~\cite{gilson1997statisticalthermodynamic,mihailescu2004theory,gallicchio2011recent,simonson2016physical,gallicchio2021comppeptsci} All the different definitions of the bound state yield nearly equivalent values for the free energy when binding is strong and specific, and the definition of the bound complex encompasses the major bound poses while excluding obvious unbound configurations. Weaker, less specific interactions, as discussed in section~\ref{sec:???} require somewhat more work to define.  In some cases, it is valuable to consider specific definitions of the complex, for example, by restricting the ligand's orientation to pinpoint the relative contributions of different binding poses.
...
\end{equation}
where $[RL]$, $[L]$, and $[R]$ are the equilibrium concentrations of complex, ligand, and receptor. One could do this by, for example, monitoring the relative population $P(RL)/P(R+L)$ of bound and unbound configurations in molecular dynamics simulations~\cite{jong2011determining,pan2017quantitative}.

Once we have defined the bound state by definining our the indicator
...
What we derived above is termed an ``absolute'' free energy.  It is still a free energy difference, as it is the difference between the ligand being in the same box of solvent as the protein, and the ligand being in it's own solvent box.

However, often what we really need to know is not the free energy of binding, but how much better one ligand is binding than another ligand.  This is really all we need to know in lead design, where we seek to find a new compund somewhat different from a referenc compound that binds more tightly.  Relative free energy calculations avoid the need to simulate binding and unbinding events by making use of the fact that the free energy is a state function and exploiting the thermodynamic cycle illustrated in Fig.~\ref{fig:fig_binding_thermodynamic_cycle}.  Fig.~\ref{fig:fig_binding_thermodynamic_cycle} tell us that the difference in freeenergy of binding between toluene(A)and benzylalcohol(B) can becomputed by running two independent calculations estimating the free energy cost of mutating A into B in the binding pocket($\Delta G_{bound}$) and in solvent($\Delta G_{unbound}$), saving us the need to simulate the physical binding process of the two compounds.  In particular, the second line of Eq. 9??? is a consequence of $\Delta G_{unbound}$ being independent of the presence of the receptor in the simulation box as the definition of the unbound state assumes receptor and ligand to be sufficiently independent for them to have no energetic interaction.
...
Let's say we have some probablity function $p_A(\vec{q})$ that we
know, expect perhaps we don't know the partition function. This is of
course the situation we are usually in with molecular simulations,
since we know that $p_A \propto e^{-u(\vec{q})}$, but we don't know
the partition function $Z_A$.
...
is just that constant.  The average 1 is just 1, and $Z_A$ and $Z_B$
...
We didn't make ant distinction between $A$ and $B$, so we can just
...
The key to this working well is that we need $\Delta_u(\vec{q})$ not
...
For transfer of a solvent from one solute to another, we can take advantage of the fact. %MRS: some more

Calculating the free energy of ligand binding is more complicated,
because there are.  We cover in section ~\ref{sec:???} how to set up
the proper partition functions in two different ways.
...
\begin{equation}
    \Delta G = k_BT \Delta f = \sum_{k=0}^{K-1} \Delta f_{k,k+1}
\end{equation}
where
\begin{equation}
    \Delta \Delta f_{k,k+1} = f_{k+1} - f_{k} = -\ln\frac{Z(\lambda_{k+1})}{Z(\lambda_{k})}
\end{equation}
